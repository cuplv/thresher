...
import com.ibm.wala.util.intset.MutableSparseIntSet;
  ...
  public class MySubGraph<T> extends AbstractNumberedGraph<T> {
	    ...
	    // WALA encodes p ->{f} q as p -> f -> q. it's not correct to remove edges p -> f and f -> q. instead, we must say that
	    ...
	    */
	    ...
	    private final class Edges implements NumberedEdgeManager<T> {
	      ...
	      public Iterator<T> getSuccNodes(T N) {
	    	  ...
	    	  while (iter.hasNext()) {
	    		  T next = iter.next();
	    		  //System.err.println("edge " + N + " " + srcNum + "  -> " + next + " " + getNumber(next));
	    		  //System.err.println(srcNum + "  -> " + getNumber(next));
	    		  
	    		  /*
	    		  if (this.lastNode != null) {
	    			  if (ignoreIfBoth.contains(getNumber(lastNode), srcNum) &&
	    				  ignoreIfBoth.contains(srcNum, getNumber(next))) {
	    				  System.err.println("ignore if both: ignoring edge " + lastNode + " -> " + N + " -> " + next);
	    				  Util.Assert(false, "ignore if both!");
	    				  continue;
	    			  } 
	    		  }
	    		  */
