...
/**
 * represents a conditional branch instruction and the paths on each of its branches (true and false) that we have seen so far 
 * @author sam
 *
 */
public class IBranchPoint {
	...
	private final SymbolTable symbolTable; // symbolTable for the method containing this point
	...
	private IBranchPoint(SSAInstruction instr, int lineNum, SSACFG.BasicBlock blk, CGNode node, boolean loopHead, int id) {
		...
		//Util.Assert(succs.size() == 2, "branch should go exactly two ways; " + blk + " goes " + succs.size());
		this.instr = instr;
	...
	public void addNewPath(IPathInfo path) {
		...
		//Util.Pre(path.getCurrentBlock().equals(this.blk), "current path is in blk " + path.getCurrentBlock() + "; branch point blk is " + this.blk);
		...
		if (trueBranch) truePaths.add(path);
		else falsePaths.add(path);
	}
	...
	public boolean addPathToLoopHead(IPathInfo path) {
		...
		for (IPathInfo oldPath : truePaths) { 
			//Util.Debug(path.query + " CONTAINS " + oldPath.query);
			// if we've seen a weakened version of this path already, don't bother adding it
			if (path.containsQuery(oldPath)) {
				//seenPaths.add(path);
				return false;
				//add = false;
				//break;
			} else if (oldPath.containsQuery(path)) {
		...
		*/
		if (truePaths.size() > 100) Util.Assert(false, "have seen an unreasonable number of paths at loop head!"); 
		return truePaths.add(path);
		//return seenPaths.add(path);
	}
	
	public boolean isLoopHead() { return loopHead; }
	...
	public String getBranchPointKey() { return branchPointKey; }
	
	public Set<IPathInfo> getTruePaths() { return truePaths; }
	
	public Set<IPathInfo> getFalsePaths() { return falsePaths; }
	
	public SSACFG.BasicBlock getBlock() { return blk; }
	
	public CGNode getMethod() { return node; }
	
	public SymbolTable getSymbolTable() { return symbolTable; }
	
	public SSAConditionalBranchInstruction getInstr() {
		Util.Assert(instr instanceof SSAConditionalBranchInstruction, "should only call this on branch points triggered by conditionals!");
	...
	@Override
	public String toString() { return this.id + ": " + this.instr + " " + this.node; }
