...
public class PointsToQuery implements IQuery {
	...
	//final TreeSet<PointsToEdge> constraints; // constraints that have not been produced yet; these need to be ordered for comparison
	final Set<PointsToEdge> constraints; // constraints that have not been produced yet
	// constraints with local LHS's that have already been produced. these do not need to be ordered for comparison
	...
	private boolean feasible = true; // this is just a sanity check to make sure that refuted queries are not re-used
	
	private int hash;
	
	
	//public PointsToQuery(Set<PointsToEdge> startConstraints, PointsToEdge producedEdge, AbstractDependencyRuleGenerator depRuleGenerator) {
	public PointsToQuery(DependencyRule startRule, AbstractDependencyRuleGenerator depRuleGenerator) {
		// create a tree set out of the input set; typically, the input set is immutable (comes from toShow set of a dep rule), so we cannot use it as our constraint set
		...
		if (producedEdge.getSource().isLocalVar()) this.produced.add(producedEdge); 
	...
	public PointsToQuery deepCopy() {
		 return new PointsToQuery(Util.deepCopyPointsToEdgeSet(constraints), 
				 				  Util.deepCopySet(produced), Util.deepCopyList(witnessList), depRuleGenerator);
	}
	
	@Override
	public boolean foundWitness() {
		return constraints.isEmpty(); // we have a witness when there are no constraints left to produce
	}
	...
	public boolean containsStaleConstraints(CGNode node) {
		for (PointsToEdge edge : constraints) {
			...
			if (edge.getSource().isLocalVar() && edge.getSource().getNode().equals(node)) {
				if (Options.DEBUG) Util.Debug("found stale constraint " + edge + "\nfor method " + node + "\nin " + this);
				this.feasible = false;
				return true;
			}
		}
	...
	public List<IQuery> visitPhi(SSAPhiInstruction instr, int phiIndex, IPathInfo currentPath) {
		...
		if (rules == null) return IQuery.FEASIBLE;
		PointerVariable matchingPhiVar = new ConcretePointerVariable(currentPath.getCurrentNode(), instr.getUse(phiIndex), this.depRuleGenerator.getHeapModel());
		...
		if (matchingRules.isEmpty()) return IQuery.FEASIBLE;
	...
	public List<IQuery> visit(SSAInstruction instr, IPathInfo currentPath) {
		Util.Assert(!(instr instanceof SSAInvokeInstruction) && !(instr instanceof SSAPhiInstruction), "instr " + instr + " should be handled elsewhere");
	...
	/**
	 * @param extraVars - list of pointer variables in another kind of constraint (such as path constraint) whose relevance we are also concerned with
	 * @return
	 */
	private List<IQuery> visitInternal(SSAInstruction instr, IPathInfo currentPath, Set<DependencyRule> rulesAtLine, Set<PointerVariable> extraVars,
									   Set<PointsToEdge> refuted) {
		if (instr instanceof SSAGetCaughtExceptionInstruction) {
			if (Options.DEBUG) Util.Debug("refuted by exceptional path"); // we assume all thrown exception bubble to the top level
			this.feasible = false;
			return IQuery.INFEASIBLE;
		}
		...
		if (Options.CHECK_ASSERTS) {
			if (Options.DEBUG) Util.Debug("CONS " + this);
			for (PointsToEdge edge1 : this.constraints) {
				if (!edge1.getSource().isLocalVar()) continue;
				for (PointsToEdge edge2 : this.constraints) {
					if (edge2.getSource().equals(edge1.getSource()) && edge1.getSink() != edge2.getSink()) {
						Util.Unimp("two occurences of local " + edge1.getSource() + " in " + this);
					}
				}
			}
		}
		*/
		...
		//Util.Assert(!rulesAtLine.isEmpty(), "non-null list should always contain rules");

		List<DependencyRule> applicableRules = new LinkedList<DependencyRule>();
		// TODO: track irrelevant rules? not doing so for now because it's inefficient to check the consistency of irrelevant rules in every case...
		// may be important to keep track of irrelevant rules because we can still get a refutation in the case where no rules are relevant, but none are consistent either
		...
		for (DependencyRule rule : rulesAtLine) {
			if (Options.DEBUG) Util.Debug("rule " + ++ruleCounter + " of " + rulesAtLine.size() + ": "+ rule);
			// see if this rule is relevant w.r.t our points-to constraints
			if (isRuleRelevant(rule, currentPath, extraVars)) {
				Set<DependencyRule> newRules = isRuleConsistent(rule, this, irrelevantRules, unsatCore, refuted, currentPath.getCurrentNode());
				if (newRules != null) {
					if (Options.DEBUG) Util.Debug("consistent: " + newRules.size());
					applicableRules.addAll(newRules);	
					unsatCore.clear();
				} else inconsistentRules++;
			}
		}
		...
		if (applicableRules.isEmpty()) {
			if (inconsistentRules != 0) {// && caseSplits.isEmpty()) {
				...
				this.feasible = false;
				return IQuery.INFEASIBLE;
			} else return IQuery.FEASIBLE; // else the current path is feasible as a "rule not applied" path or no applicable rules were found
		} else if (applicableRules.size() == 1) {
			DependencyRule rule = applicableRules.get(0);
			//PointsToQuery copy = this.deepCopy();
			if (!Options.ABSTRACT_DEPENDENCY_RULES) caseSplits.add(this.deepCopy()); // add "rule not applied" path. this is now done elsewhere, no longer needed
			applyRule(rule, this);
			return caseSplits;
		} else { // many applicable rules
			LinkedList<IQuery> cases = new LinkedList<IQuery>();
			boolean first = true;
			PointsToQuery copy = this.deepCopy(); // need to make copy to make other copies of 
			if (Options.DEBUG) Util.Debug(applicableRules.size() + " applicable rules");
			if (Options.DEBUG) for (DependencyRule rule : applicableRules) { Util.Debug("rule " + rule); }
			...
			for (DependencyRule rule : applicableRules) {
				if (first) {
					applyRule(rule, this);
					first = false;
					continue; // first query will be the path we continue on (not added to case split array) 
				}
				PointsToQuery curQuery = copy.deepCopy(); // create copy for this case and add it to case split array)
				applyRule(rule, curQuery);
				cases.addLast(curQuery);
			}
			if (Options.DEBUG) Util.Debug("returning " + cases.size() + " cases.");
			return cases;
		}
	}
	 ...
	 * @param instr - call instruction that we have just finished executing
	 * @param callee - target of call instruction that we have just finished executing
	 */
	@Override
	public List<IQuery> returnFromCall(SSAInvokeInstruction instr, CGNode callee, IPathInfo currentPath) {
		List<IQuery> caseSplits;
		if (currentPath.isCallStackEmpty()) { // special case; entering entirely new node rather than returning to node we were already in
			if (Options.GEN_DEPENDENCY_RULES_EAGERLY) depRuleGenerator.generateRulesForNode(currentPath.getCurrentNode());
			...
			if (!isFeasible()) return IQuery.INFEASIBLE; // check for refutation--can refute based on null bindings in the above
		} else caseSplits = IQuery.FEASIBLE;

		
		// we need to do this in case there were rules that we didn't apply when entering the call because they weren't relevant, but they are relevant now
		// TODO: is this heavy-handed? maybe we can just intellegently drop constraints rather than re-entering the call...
		caseSplits = enterCall(instr, callee, currentPath);
		if (caseSplits == IQuery.INFEASIBLE) {
			this.feasible = false;
			return IQuery.INFEASIBLE;
		}
		if (Options.CHECK_ASSERTS) Util.Assert(caseSplits.isEmpty(), "shouldn't case split here because the arguments to the call are known!");
		
		List<PointsToEdge> toRemove = new LinkedList<PointsToEdge>();
		for (PointsToEdge edge : produced) { 
			if (edge.getSource().getNode().equals(callee)) toRemove.add(edge);
		}
		for (PointsToEdge edge : toRemove) {
			boolean removed = produced.remove(edge);
			if (Options.CHECK_ASSERTS) Util.Assert(removed, "couldn't remove edge " + edge);
		}
		
		if (this.containsStaleConstraints(callee)) {
			if (Options.PIECEWISE_EXECUTION) {
				// TODO: hack! can make piecewise execution a bit more careful about adding/removing local constraints
		...
		//Util.Post(!this.con	tainsStaleConstraints(callee), "should not contain stale constraints after substitution!");
		return IQuery.FEASIBLE;
	}
	
	/**
	 * bind formals to actuals that appear in the LHS of our current constraints, and add resulting constraint to produced set
	...
	Set<PointerVariable> bindActualsToFormals(SSAInvokeInstruction instr, CGNode callerMethod, CGNode calleeMethod) {
		...
		for (int i = 0; i < instr.getNumberOfParameters(); i++) {
			...
			// necessary for cases when we pass the same value as two parameters i.e. Object o; foo(o, o).
		...
		if (!possiblyAliased.isEmpty()) { // handle case where aliasing relationship may be created by param passing
			if (Options.DEBUG) Util.Debug("potential aliasing!");
			for (PointerVariable var : possiblyAliased) {
				Set<PointsToEdge> aliasedEdges = new TreeSet<PointsToEdge>(); // set of edges with var as LHS
				for (PointsToEdge edge1 : this.constraints) {
					if (edge1.getSource().isLocalVar() && edge1.getSource().equals(var)) aliasedEdges.add(edge1);
				}					
				toRemove.clear();
				toAdd.clear();
				for (PointsToEdge edge1: aliasedEdges) { // for each edge with the same LHS
					for (PointsToEdge edge2: aliasedEdges) {
						...
						if (!edge1Sink.isSymbolic() && !edge2Sink.isSymbolic()) { // both concrete
							...
							if (edge1Sink.symbContains(edge2Sink)) toRemove.add(edge1); 
							else if (edge2Sink.symbContains(edge1Sink)) toRemove.add(edge2);
							else if (Util.intersectionNonEmpty(edge1Sink.getPossibleValues(), 
															   edge2Sink.getPossibleValues())) {
								...
								toAdd.add(new PointsToEdge(edge1.getSource(), SymbolicPointerVariable.makeSymbolicVar(intersectedValues)));//new SymbolicPointerVariable(intersectedValues)));
							} else {
				...
				for (PointsToEdge edge : toAdd) addConstraint(edge);
			}
		}
	...
	Set<PointerVariable> bindFormalsToActuals(SSAInvokeInstruction instr, CGNode callerMethod, CGNode calleeMethod) {
		// TODO: optimization: the receiver of a constructor must have the same type as the obj it's constructing. we can refute even earlier in this case
		...
		if (instr.hasDef()) {
			...
			PointerVariable callerReturnValue = new ConcretePointerVariable(callerMethod, instr.getDef(), this.depRuleGenerator.getHeapModel());
			...
			for (PointsToEdge edge : toRemove) Util.Assert(constraints.remove(edge), "Couldn't remove edge " + edge + " from " + this);
			for (PointsToEdge edge : toAdd) Util.Assert(addConstraint(edge), "Couldn't add edge " + edge);
		}
		...
		for (int i = 0; i < instr.getNumberOfParameters(); i++) {
			...
			for (PointsToEdge edge : constraints) {
				if (edge.getSource().equals(arg)) {
						...
						if (prod.getSource().equals(formal) && !prod.equals(newEdge)) {
							...
							if (prod.getSink().isSymbolic() && !newEdge.getSink().isSymbolic()) {
								...
								if (sink.getPossibleValues().contains(newEdge.getSink().getInstanceKey())) {
									...
									toRemove.add(prod);
								} else Util.Assert(false, "incompatible multiple assignments to " + formal + " edge " + newEdge + " produced " + 
												   Util.constraintSetToString(produced));
							}
						}
						//Util.Assert(!prod.getSource().equals(formal) || prod.equals(newEdge), 
							//		"multiple assignments to " + formal + " edge " + newEdge + " produced " + Util.constraintSetToString(produced));
					}
					...
					// END DEBUG
					for (PointsToEdge removeMe : toRemove) Util.Assert(produced.remove(removeMe), "couldn't remove edge " + removeMe);
					toRemove.clear();
	...
	public List<IQuery> enterCall(SSAInvokeInstruction instr, CGNode callee, IPathInfo currentPath, Set<PointerVariable> extraVars) {
		...
		CGNode caller = currentPath.getCurrentNode(); // the call instruction occurs in the caller 
		// need to do this even if there are no rules at the line because the return value may be important (and is not represented in the dependency rules)
		...
		// set of function parameters we have seen for which there is a relevant but not consistent rule
		...
		for (DependencyRule rule : rulesAtLine) { // create map of rules to parameters
			//Util.Assert(calleeMethod.getSignature().equals(
				//		rule.getShown().getSource().getNode().getMethod().getSignature()), "method signatures don't match! " + 
			...
			if (isRuleRelevant(rule, currentPath, extraVars)) {
				...
				Set<DependencyRule> newRules = isRuleConsistent(rule, this, new TreeSet<DependencyRule>(), this.unsatCore, EMPTY_PT_EDGE_SET, null);
				if (newRules != null) {
				//if (isRuleConsistent(rule, this, caseSplits, new TreeSet<DependencyRule>(), EMPTY_PT_EDGE_SET, null)) {
				...
				} else if (!assignedParams.contains(paramId)) inconsistentParams.add(paramId);
			}
		}
		...
		for (List<DependencyRule> rulesForParam : idRuleMap.values()) {
			if (rulesForParam.size() > 1) {
				if (idRuleMap.keySet().size() == 1) {
					...
					for (DependencyRule rule : rulesForParam) applicableRules.add(rule);
					break;
				} // else, this is tricky. more than one parameter, and more than one choice for one of them
				Util.Unimp("more than one rule for multiple parameters");
			} else if (rulesForParam.size() == 1) applicableRules.add(rulesForParam.get(0));
		}
	...
	static void applyRule(DependencyRule rule, PointsToQuery query) {
		...
		//Util.Pre(query.constraints.contains(rule.getShown()), "produced edge " + rule.getShown() + " not in query! " + query);
		
		/*
		if (shownLHS.isLocalVar()) {
			List<PointsToEdge> toRemove = new LinkedList<PointsToEdge>();
			// check for previous occurrences of this variable and remove - we're doing a strong update
			for (PointsToEdge edge : query.constraints) {
				if (edge.getSource().equals(shownLHS)) toRemove.add(edge);
			}
			for (PointsToEdge edge : toRemove) query.constraints.remove(edge);
		}
		*/
		

		List<PointsToEdge> toRemove = new LinkedList<PointsToEdge>();
		// special case for when the constraints contain a symbolic edge that matches a concrete edge in the constraints
			...
			for (PointsToEdge edge : query.constraints) {
				if (edge.getSink().isSymbolic() && edge.getSource().equals(ruleEdge.getSource())) {
					if (edge.getSink().symbContains(ruleEdge.getSink())) {
						// remove the symbolic edge; it should be replaced with the concrete one from this rule
						toRemove.add(edge);
					}
				}
			}
		...
		//Util.Assert(removed, "couldn't remove edge " + rule.getShown() + " from " + query);
		
		for (PointsToEdge edge : rule.getToShow()) {
			// don't want to re-add edges with local LHS's
			if (!edge.getSource().isLocalVar() || !query.produced.contains(edge)) query.addConstraint(edge);
		}
		
		if (Options.LOG_WITNESSES) { query.witnessList.add(rule); }
		
		// only want to add constraints with local LHS to shown set because we can produced refutations based on shown set...
		// ... it would be unsound to do this if we used constraints with heap LHS (could be a different instance or strong update)
	...
	/**
	 * match symbolic variables in dependency rules with concrete/symbolic vars already present in our constraints
	 * @param rule
	 * @param currentQuery
	 * @param currentNode
	 * @return
	...
	Set<DependencyRule> bindSymbolicRule(DependencyRule rule, PointsToQuery currentQuery, CGNode currentNode) {
		...
		//Map<SymbolicPointerVariable,PointerVariable> subMap = new HashMap<SymbolicPointerVariable,PointerVariable>(); 
		...
		for (PointsToEdge edge : constraints) {
			for (PointsToEdge ruleEdge : ruleEdges) {
				if (ruleEdge.getSource().isLocalVar() && edge.getSource().isLocalVar() && 
					!edge.getSink().isSymbolic()) ruleEdge.getSubsFromEdge(edge, subMaps, alreadySubbed, true);
			}
		}
		for (PointsToEdge edge : produced) {
			Util.Assert(edge.getSource().isLocalVar(), "produced should only contain locals");
			for (PointsToEdge ruleEdge : ruleEdges) {
				if (ruleEdge.getSource().isLocalVar() && !edge.getSink().isSymbolic()) ruleEdge.getSubsFromEdge(edge, subMaps, alreadySubbed, true);
			}
		}
		...
		for (PointsToEdge edge : constraints) {
			for (PointsToEdge ruleEdge : ruleEdges) {
				if (ruleEdge.getSource().isLocalVar() && edge.getSink().isSymbolic()) ruleEdge.getSubsFromEdge(edge, subMaps, alreadySubbed, false);
			}
		}
		for (PointsToEdge edge : produced) {
			for (PointsToEdge ruleEdge : ruleEdges) {
		...
		Util.Assert(subMaps.size() == 1, "more than one instantiation choice for shown! have " + subMaps.size() + " this shouldn't happen, since we've only considered local constraints");
		...
		for (PointsToEdge edge : constraints) {
			for (PointsToEdge ruleEdge : ruleEdges) {
				if (!ruleEdge.getSource().isLocalVar() && !edge.getSource().isLocalVar()) ruleEdge.getSubsFromEdge(edge, subMaps, alreadySubbed, false);
			}
		}
		...
		for (DependencyRule symbRule : rules) {
			...
			if (symbVars.size() > 1) { // more than 1 symbolic var; consider all possible aliasing combos
				...
				if (rule.getSymbolicVars().contains(var0)) { 
					if (Options.DEBUG) Util.Debug("considering aliasing of " + var0 + " and " + var1);
					aliasMap.put(var0, var1);
				}
				else {
					if (Options.DEBUG) Util.Debug("considering aliasing of " + var1 + " and " + var0);
	...
	Set<DependencyRule> bindSymbolicRule(DependencyRule rule, PointsToQuery currentQuery, CGNode currentNode) {
		...
		// match edges with local LHS's first. since these can't have case splits, they are hard constraints
		for (PointsToEdge edge : constraints) {
			for (PointsToEdge ruleEdge : ruleEdges) {
				if (ruleEdge.getSource().isLocalVar() && edge.getSource().isLocalVar()) ruleEdge.getSubsFromEdge(edge, subMaps, alreadySubbed, true);
			}
		}
		...
		for (PointsToEdge edge : produced) {
			Util.Assert(edge.getSource().isLocalVar(), "produced should only contain locals");
			for (PointsToEdge ruleEdge : ruleEdges) {
				if (ruleEdge.getSource().isLocalVar()) ruleEdge.getSubsFromEdge(edge, subMaps, alreadySubbed, true);
			}
		}
		Util.Assert(subMaps.size() == 1, "more than one instantiation choice for shown! have " + subMaps.size() + " this shouldn't happen, since we've only considered local constraints");
		//DependencyRule newRule = rule;
	...
	*/
		...
		for (PointsToEdge constraint : constraints) { 
			for (PointsToEdge edge : checkMe) {
				...
				if (edge.getSource().isSymbolic() && constraint.getSource().isSymbolic()) { // both edges symbolic
					//lhsMatch = Util.intersectionNonEmpty(edge.getSource().getPossibleValues(), constraint.getSource().getPossibleValues());
					lhsMatch = edge.getSource() == constraint.getSource();
					fieldsEqualAndNotArrays = Util.equal(edge.getFieldRef(), constraint.getFieldRef()) &&
	  						  (edge.getFieldRef() == null ||
	  					      !(edge.getFieldRef() == AbstractDependencyRuleGenerator.ARRAY_CONTENTS)) &&
	  						  (constraint.getFieldRef() == null ||
	  					      !(constraint.getFieldRef() == AbstractDependencyRuleGenerator.ARRAY_CONTENTS));
					/*
					if (lhsMatch && fieldsEqualAndNotArrays) {
						PointerVariable sub = subMap.get(edge.getSource());
						Util.Assert(sub == null || sub.equals(constraint.getSource()), 
									"more than one instantiation choice for " + edge.getSource() + ": " + sub + " and " + constraint.getSource());
						if (sub == null) {
							Util.Debug("adding sub relationship " + edge.getSource() + " " + constraint.getSource());
							subMap.put((SymbolicPointerVariable) edge.getSource(), constraint.getSource());
						}
					}
					*/
					...
					fieldsEqualAndNotArrays = Util.equal(edge.getFieldRef(), constraint.getFieldRef()) &&
	  						  (edge.getFieldRef() == null ||
	  					      !(edge.getFieldRef() == AbstractDependencyRuleGenerator.ARRAY_CONTENTS)) &&
	  						  (constraint.getField() == null ||
	  					      !(constraint.getField() instanceof ArrayContentsKey));
					/*
					if (lhsMatch && fieldsEqualAndNotArrays) {
						PointerVariable sub = subMap.get(edge.getSource());
						Util.Assert(sub == null || sub.equals(constraint.getSource()), 
									"more than one instantiation choice " + sub + " " + constraint.getSource());
						if (sub == null) {
							subMap.put((SymbolicPointerVariable) edge.getSource(), constraint.getSource());
						}
					}
					*/
					...
					fieldsEqualAndNotArrays = Util.equal(edge.getFieldRef(), constraint.getFieldRef()) &&
	  					      ...
	  					      !(constraint.getFieldRef() == AbstractDependencyRuleGenerator.ARRAY_CONTENTS)) &&
	  						  (edge.getField() == null ||
	  					      !(edge.getField() instanceof ArrayContentsKey));
				...
				if (lhsMatch && fieldsEqualAndNotArrays) {
					...
					if (edge.getSink().isSymbolic() && constraint.getSink().isSymbolic()) {
						//rhsEq = Util.intersectionNonEmpty(edge.getSink().getPossibleValues(), constraint.getSink().getPossibleValues());
						rhsEq = edge.getSink() == constraint.getSink();
						/*
						PointerVariable sub = subMap.get(edge.getSink());
						Util.Assert(sub == null || sub.equals(constraint.getSink()), 
													"more than one instantiation choice " + sub + " " + constraint.getSink());
						if (sub == null) {
							Util.Debug("adding sub relationship " + edge.getSink() + " " + constraint.getSink());
							subMap.put((SymbolicPointerVariable) edge.getSink(), constraint.getSink());
						}
						*/
					...
					if (!rhsEq) {
						if (Options.DEBUG) Util.Debug("refuted: " + edge + " and " + constraint);
	...
	boolean isRuleRelevant(DependencyRule rule, IPathInfo currentPath, Set<PointerVariable> extraVars) {
		...
		for (PointsToEdge edge : constraints) {
			for (PointsToEdge toCheck : checkMe) {
				boolean match = edge.getSource().equals(toCheck.getSource()) && // lhs's match
		...
		// also possible that it matches / contradicts a constraint we already have in produced
		for (PointsToEdge edge : produced) {
			Util.Assert(edge.getSource().isLocalVar(), "only constraints with local LHS should be in produced set!");
			for (PointsToEdge toCheck : checkMe) {
				if (toCheck.getSource().isLocalVar() && // for produced, can only refuted based on locals... heap locs can be mutated 
					...
					if (!WALACFGUtil.isInLoopBody(currentPath.getCurrentBlock(), currentPath.getCurrentNode().getIR())) { 
						...
						return true;
					} // else, this assignment is in a loop, where multiple assigns can occur. not relevant to us
				}
			}
		}
		
		/*
		// check for relevance to extra vars
		for (PointsToEdge edge : checkMe) {
			if (extraVars.contains(edge.getSource())) {
				if (Options.DEBUG) Util.Debug("relevant (extra) " + edge);
				return true;
			}
		}*/
	...
	/*
	boolean isRuleConsistent(DependencyRule rule, PointsToQuery currentQuery, List<IQuery> caseSplits, Set<DependencyRule> irrelevantRules,
							 Set<PointsToEdge> refuted, CGNode currentNode) {
		// must be consistent both with current constraints and constraints we have already produced
		//if (!isRuleConsistent(rule, currentQuery, caseSplits, this.constraints, irrelevantRules, refuted, currentNode)) return false;
		//return isRuleConsistent(rule, currentQuery, caseSplits, this.produced, irrelevantRules, refuted, currentNode);
		if (!isSymbolicRuleConsistent(rule, currentQuery, this.constraints, currentNode)) return false;
		return isSymbolicRuleConsistent(rule, currentQuery, this.produced, currentNode);
	}
	*/
		...
		if (Options.ABSTRACT_DEPENDENCY_RULES) {
			if (rule.isSymbolic()) {
				...
				for (DependencyRule newRule : newRules) {
					...
					//DependencyRule newRule = isSymbolicRuleConsistent(rule, currentQuery, this.constraints, currentNode);
						toRemove.add(newRule);
						continue;
					}
					//if (isSymbolicRuleConsistent(rule, currentQuery, this.produced, currentNode) == null) return null;
				...
				if (newRules.isEmpty()) return null; 
				return newRules;
			} else if (rule.getStmt() != null && rule.getStmt().isNewInstr()){ // this is a new instruction. bind it
				PointsToEdge shown = rule.getShown();
				...
				//Util.Unimp("shouldn't get here for rule " + rule + " query " + currentQuery);
				singleton.add(rule);
				return singleton;
			} else return doConsistencyCheckNonSymbolic(rule, currentQuery, irrelevantRules, unsatCore, refuted, currentNode);
		} else {
			return doConsistencyCheckNonSymbolic(rule, currentQuery, irrelevantRules, unsatCore, refuted, currentNode);
		}
	}
	
	private Set<DependencyRule> doConsistencyCheckNonSymbolic(DependencyRule rule, PointsToQuery currentQuery, Set<DependencyRule> irrelevantRules,
			 								   List<PointsToEdge> unsatCore, Set<PointsToEdge> refuted, CGNode currentNode) {
		if (!isRuleConsistent(rule, currentQuery, this.constraints, unsatCore, irrelevantRules, refuted, currentNode)) return null;
	...
	static boolean isRuleConsistent(DependencyRule rule, PointsToQuery currentQuery, Set<PointsToEdge> constraints, 
		...
		/*
		for (PointsToEdge edge : refuted) {
			if (checkMe.contains(edge)){
				Util.Unimp("refuting based on this");
				Util.Debug("edge " + edge + " aready refuted!");
				return false; 
			}
		}
		*/
		...
		for (PointsToEdge constraint : constraints) {
			for (PointsToEdge edge : checkMe) {
				// to check: lhs's equal and (fieldnames equal and not array's) => rhs's equal
				...
				if (lhsNameMatch && fieldsEqualAndNotArrays && !rhsesEqual) {
					...
					if (constraintSrc.isLocalVar()) { // can't split a local into multiple instances...
						if (WALACFGUtil.isInLoopBody(rule.getBlock(), rule.getNode().getIR())) { // ...unless it occurs in a loop
						//if (Util.isEdgeProduceableByLoop(edge, rule.getNode(), currentQuery.depRuleGenerator.getHeapGraph(), 
														 //currentQuery.depRuleGenerator, currentQuery.depRuleGenerator.getCallGraph())) {
							// (implicitly) allow both locals to be added; a strong update will be enforced when the rule is applied
							//continue;
							//Util.Log("local refuted: " + edge + " and " + constraint);
							//return false;
						} else {
							// locals are not in a loop. we have a definite refutation, since no local can point to two values at once
							Util.Debug("refuted: " + edge + " and " + constraint);
							unsatCore.add(constraint);
							return false;
						}
					}
					// else, LHS is a heap loc. we can't definitely refute, since they may be different instances of the same abstract location
					//Set<Integer> instanceNums = collectInstanceNumsFor(constraintSrc, constraints);
					//Util.Assert(instanceNums.size() == 1, "more than one instance num? this is unexpected...");
					// TODO: no longer need this case...I think case splits for phi's suffice to handle this
					// if there's only one instance num for the var in question, then it suffices to consider two cases: one where the rule is applied, 
					// and one where it is not. otherwise, we must do a case split for each instance number...
					//if (caseSplits.isEmpty()) caseSplits.add(currentPath.deepCopy()); // a single "rule not applied" case suffices
					Util.Debug("refuted: " + edge + " and " + constraint);
					unsatCore.add(constraint);
					return false; // ok to label as inconsistent; only the rule not taken path (already accounted for) is consistent
				}
			}
		}
		/*
		if (currentNode == null) return true;
		if (rule.getShown().getSource().isLocalVar()) Util.Assert(rule.getShown().getSource().getNode().equals(currentNode), 
																"nodes don't match: cur " + currentNode + " and rule " + 
																 rule.getShown().getSource().getNode() + "\nfor rule  " + rule);
		for (PointsToEdge edge : rule.getToShow()) {
			if (edge.getSource().isLocalVar()) Util.Assert(edge.getSource().getNode().equals(currentNode), 
					"nodes don't match: cur " + currentNode + " and rule " + edge.getSource().getNode() + "\nfor rule " + rule);
		}
		*/
		//Util.Post(caseSplits.isEmpty(), "this is not handled correctly in all callers; they expect it to be empty. fix this");
		return true;
	}
	 ...
	 */
	/*
	static Set<Integer> collectInstanceNumsFor(PointerVariable var, Set<PointsToEdge> constraints) {
		Util.Assert(var.isHeapVar(), "Only heap vars can have instance num!");
		Set<Integer> instanceNumSet = new HashSet<Integer>();

		for (PointsToEdge constraint : constraints) {
			PointerVariable src = constraint.getSource(), snk = constraint.getSink();
			if (var.equals(src)) {
				instanceNumSet.add(src.getInstanceNum());
			} 
			
			if (var.equals(snk)) {
				instanceNumSet.add(snk.getInstanceNum());
			}
		}
		return instanceNumSet;
	}
	*/
	
	/**
	 * find the var pointed to by the given var in our consraints
	 * @param var - variable to look for on LHS of points-to relation
	 * @return return the location pointed to by var, or null if var is not found on the LHS of any constraint in the set
	 */
	 ...
	 * @param var - variable to look for on LHS of points-to relation
	 * @param set - set of edges to search in
	 * @return return the location pointed to by var, or null if var is not found on the LHS of any constraint in the set
	 */
	...
	@Override
	public boolean addConstraintFromBranchPoint(IBranchPoint point, boolean trueBranchFeasible) { return true; }
	...
	public boolean isCallRelevant(SSAInvokeInstruction instr, CGNode caller, CGNode callee, CallGraph cg) {
		if (instr.hasDef()) {
			...
			for (PointsToEdge edge : this.constraints) {
				if (edge.getSource().equals(retval)) {
					if (Options.DEBUG) Util.Debug("retval relevant: " + edge + "\nand\n" + retval);
		...
		Set<DependencyRule> rulesProducedByCallee = Util.getRulesProducableByCall(callee, cg, depRuleGenerator);
		for (DependencyRule rule : rulesProducedByCallee) {
			PointerVariable shownLHS = rule.getShown().getSource();
			for (PointsToEdge edge : this.constraints) {
				if (edge.getSource().equals(shownLHS)) {
		...
		*/
		...
		for (PointsToEdge edge : constraints) {
			...
			if (key != null && keys.contains(key)) {
				//Util.Assert(relevant, "mod/ref and dep rules disagree on relevance of " + edge);
				return true; 
			}
		}
	...
	public void dropConstraintsProduceableInCall(SSAInvokeInstruction instr, CGNode caller, CGNode callee) {
		...
		if (instr.hasDef()) {
			ConcretePointerVariable retval = new ConcretePointerVariable(caller, instr.getDef(), this.depRuleGenerator.getHeapModel());
			for (PointsToEdge edge : this.constraints) {
				if (edge.getSource().equals(retval)) toRemove.add(edge);
			}
		}
		
		/*
		// collect all dependency rules produceable by this call
		Set<DependencyRule> rulesProducedByCallee = Util.getRulesProducableByCall(callee, depRuleGenerator.getCallGraph(), depRuleGenerator);
		for (PointsToEdge edge : this.constraints) {
			PointerVariable src = edge.getSource();
			if (retval != null && src.equals(retval)) {
				toRemove.add(edge);
				continue;
			}
			for (DependencyRule rule : rulesProducedByCallee) {
				PointerVariable shownLHS = rule.getShown().getSource();
				if (src.equals(shownLHS)) toRemove.add(edge);
			}
		}
		*/		
		...
		for (PointsToEdge edge : constraints) {
			PointerKey key = edge.getField();
			if (key != null && keys.contains(key)) toRemove.add(edge);
		}
	...
	public boolean containsLoopProduceableConstraints(SSACFG.BasicBlock loopHead, CGNode currentNode) {
		...
		Set<DependencyRule> rules = depRuleGenerator.getRulesForNode(currentNode); // get all rules for node
		...
		for (DependencyRule rule : loopRules) { // see if any of the rules produces one of our edges
			if (this.constraints.contains(rule.getShown())) return true;
			
			else {
				for (PointsToEdge constraint : constraints) {
					if (constraint.symbEq(rule.getShown())) return true;
				}
			}
		}
		...
		for (CGNode callNode : targets) { // drop all vars that can be written by a call in the loop body
			OrdinalSet<PointerKey> callKeys = depRuleGenerator.getModRef().get(callNode);
			for (PointsToEdge edge : constraints) {
				PointerKey key = edge.getField();
				if (key != null && callKeys.contains(key)) return true;
			}
		}
		/*
		for (PointsToEdge edge : this.constraints) {
			if (edge.getSource().isLocalVar() &&
					Util.isEdgeProduceableByLoop(edge, loopHead, currentNode, depRuleGenerator.getHeapGraph(), depRuleGenerator, depRuleGenerator.getCallGraph())) {
				Util.Debug("produceable: " + edge);
				return true;
			}
		}
		*/
	...
	public void removeLoopProduceableConstraints(SSACFG.BasicBlock loopHead, CGNode currentNode) {
		...
		for (PointsToEdge edge : toDrop) this.constraints.remove(edge);
		toDrop.clear();
				
		// the loop may also contain callees. drop any constraint containing vars that these callees can write to
		Set<CGNode> targets = WALACFGUtil.getCallTargetsInLoop(loopHead, currentNode, depRuleGenerator.getCallGraph());
		
		for (CGNode callNode : targets) { // drop all edges that can be written by a call in the loop body
			OrdinalSet<PointerKey> callKeys = depRuleGenerator.getModRef().get(callNode);
			for (PointsToEdge edge : this.constraints) {
				if (edge.getField() != null && callKeys.contains(edge.getField())) toDrop.add(edge);
			}
			for (PointsToEdge edge : toDrop) this.constraints.remove(edge);
			toDrop.clear();
		}		
	...
	public boolean contains(IQuery other) {
		if (!(other instanceof PointsToQuery)) return false;
		PointsToQuery otherQuery = (PointsToQuery) other;
		
		if (otherQuery.constraints.isEmpty()) return true; 
		// can't contain the other query if it's bigger than us
		if (otherQuery.constraints.size() > this.constraints.size()) return false;
		
		//Util.Debug("asking if " + this.constraints + " contains " + otherQuery.constraints);
		
		for (PointsToEdge constraint1 : otherQuery.constraints) { // for each constraint in other
			boolean contained = false;
			for (PointsToEdge constraint2 : this.constraints) { // for each constraint in this
				//Util.Debug("eq " + constraint1 + " and " + constraint2);
				if (constraint2.symbContains(constraint1)) { // if this contains the constraint from other
	...
	public int compareTo(Object other) {
		if (!(other instanceof PointsToQuery)) Util.Unimp("comparing to non- points-to query " + other.getClass() + " " + other);
		PointsToQuery otherQuery = (PointsToQuery) other;
		TreeSet<PointsToEdge> otherConstraints = otherQuery.constraints;
		final int ptSize = constraints.size(), otherPtSize = otherConstraints.size();
		if (ptSize > otherPtSize) return 1;
		else if (ptSize < otherPtSize) return -1;
		// sizes are the same

		// sizes are the same; do edge-by-edge comparison
		final Iterator<PointsToEdge> ptIter = constraints.descendingIterator();
		final Iterator<PointsToEdge> otherPtIter = otherConstraints.descendingIterator();
		while (ptIter.hasNext() && otherPtIter.hasNext()) {
			final PointsToEdge edge0 = ptIter.next();
			final PointsToEdge edge1 = otherPtIter.next();
			final int comparison = edge0.compareTo(edge1);
			if (comparison != 0) return comparison;
		}
		return 0; // all edges compared equal
	}
	*/
	...
	public void removeAllLocalConstraints() {
		List<PointsToEdge> toRemove = new LinkedList<PointsToEdge>();
		for (PointsToEdge edge : constraints) {
			if (edge.getSource().isLocalVar()) toRemove.add(edge);
		}
	...
	//public Set<CGNode> getMethodsRelevantToQuery() {
		...
		for (PointsToEdge edge : constraints) {
			...
			if (edge.isSymbolic()) {
				...
				} else Assertions.UNREACHABLE();
			} else producingRules.addAll(Util.getProducersForEdge(edge, depRuleGenerator));
			//Set<DependencyRule> producingRules = Util.getProducersForEdge(edge, depRuleGenerator);
			// TODO: this means edge is definitely infeasible, and thus so are constraints... should do something about this
			//Util.Assert(!producingRules.isEmpty(), "couldn't find producer for edge " + edge);
	...
	public void intializeStaticFieldsToDefaultValues() {
		for (PointsToEdge edge : this.constraints) {
			// if a constraint required a static field to point to something, that constraint will never be satisfied. refute.
			if (edge.getSource().getInstanceKey() instanceof StaticFieldKey) {
				if (Options.DEBUG) Util.Debug("refuted by default values for static fields!");
				this.feasible = false;
				return;
			}
		}
	...
	public boolean addContextualConstraints(CGNode node, IPathInfo currentPath) {
		// TODO: for now, we only need to make receiver constraints explicit. we may have a need for others in the future as well...
		...
		if (receiver instanceof InstanceKey) {
			...
			DependencyRule rule = new DependencyRule(receiverConstraint, null, new TreeSet<PointsToEdge>(), node, node.getIR().getControlFlowGraph().entry());
			Set<DependencyRule> newRules = isRuleConsistent(rule, this, new TreeSet<DependencyRule>(), unsatCore, EMPTY_PT_EDGE_SET, currentPath.getCurrentNode());
			if (newRules != null) { // && !produced.contains(receiverConstraint)) {
				...
				if (Options.DEBUG) Util.Debug("refuted by contextual constraints. newRules " + newRules);
				return false;
			}
		} 
		/*
		ContextItem caller = context.get(ContextKey.CALLER);
		if (caller instanceof CGNode) {
	...
	/**
	 * perform non-destructive set differencing between start constraints and this (i.e. do startConstraints \ this)
	 */
	//@Override
	...
	public List<DependencyRule> getWitnessList() { return witnessList; }
	
	@Override
	public boolean containsConstraint(Constraint constraint) {
		if (!(constraint instanceof PointsToEdge)) return false;
	...
	public boolean equals(Object other) {
		if (!(other instanceof PointsToQuery)) return false;
		PointsToQuery otherQuery = (PointsToQuery) other;
		return constraints.equals(otherQuery.constraints);
	}
