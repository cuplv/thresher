...
public class Logger {
	...
	public Logger(String benchmark) { this.benchmark = benchmark; }
	
	public void log(String str) {
		Integer count = countMap.get(str);
		int countValue;
		if (count == null) countValue = 0;
		else countValue = count.intValue();
	...
	public String dumpCSV() {
		// only reporting timeouts on errors where error was witnessed (since precision loss doesn't matter if we refuted the error)
	...
	public void logPathStackSize(int pathStackSize) {
		if (pathStackSize > this.maxPathStackSize) this.maxPathStackSize = pathStackSize;
	}
	...
	public String dumpHumanReadable() {
		return totalErrors + " totals errors\n" + totalFields + " total fields\n" + errorsRefuted + " errors refuted\n" + errorsWitnessed + " errors witnessed\n" + edgesRefuted + " edges refuted\n" + 
			   + (totalFields - witnessedFields.size()) + " fields refuted\n" + witnessedFields.size() + " fields witnessed\n" +
