...
/**
 * a very simple symbolic executor. interprocedurally explores all program paths in DFS fashion. 
 * conditional branch instructions are treated as nondeterministic choices
 * loops are unrolled once, though all (non-looping) paths through the loop body are explored
 * @author sam
 */
public class BasicSymbolicExecutor implements ISymbolicExecutor {
	...
	/**
	 * @param startNode - CGNode from which to begin
	 * @param query - fact that symbolic execution will witness or refute 
	...
	public boolean executeBackward(CGNode startNode, SSACFG.BasicBlock startBlk, int startLine, IQuery query) {
		...
		//Util.visualizeIR(Options.DEBUG_cha, startNode.getIR(), "TEST"); // DEBUG only; can get a view of weird IR if it's giving us trouble
	...
	public boolean executeBackward() {
		...
		for (;;) {
			...
			boolean oom = false;
			if (Runtime.getRuntime().freeMemory() / Runtime.getRuntime().totalMemory() >= 0.95) oom = true;  
			...
			//if (path.getPathId() != lastPath) { // prevent printing except when we pick a new path (logging parsimony issue, not correctness issue)
				if (Options.DEBUG) Util.Debug("executing path " + path.getPathId() + "X");
				if (Options.DEBUG) Util.Debug(path.toString());
			...
			//if (!path.isFeasible()) continue; // refuted. this case shouldn't be needed; executeBWPath should never return true for an infeasible path.
			if (hitProcBoundary) {
			...
			} // else, path is infeasible, either because it split or because it was refuted. execute next path
		}
	}
	 ...
	 * we do not attempt to model the order that static initializers are invoked in, so we just (soundly) punt here... 
	 * fancier symbolic executors can override and do better
	 */
	...
	// punt!
	boolean handleFakeRootMethod(IPathInfo path, CGNode entrypoint) { return true; }
			...
			if (Options.SUBSUMPTION_CHECK_AT_SUMMARIES) {
				...
				for (IPathInfo seenPath : seen) {
					if (path.containsQuery(seenPath)) { // have we seen a simpler path?
						if (Options.DEBUG) Util.Debug("refuted by function summary;\n" + seenPath + "\nsimpler than\n" + path);
						path.refute();
						return true;
	...
	/**
	 * exposed to allow the inclusion of heuristics for not exploring certain callers
	 * this implementation uses no heuristics and simply returns all callers 
	 */
	@Override
	...
	public boolean handleInterproceduralExecution(IPathInfo path) {
		...
		if (this.callGraph.getEntrypointNodes().contains(callee) && this.callGraph.getPredNodeCount(callee) == 1) {
			...
			// first, try getting easy witness by going straight from entrypoint to fakeWorldClinit
			...
			if (handleFakeRootMethod(path, entrypoint)) {//iif (handleFakeWorldClinit(path)) {
				path.declareFakeWitness(); // TODO: this is not a fake witness. the problem is that newPath can split into multiple paths and we can find a witness on a different one
		...
		if (path.getCurrentNode().getMethod().isClinit()) {
			...
			// current node is a class initializer... our only caller can be fakeWorldClinit. handling this is a special case
			if (handleFakeWorldClinit(path)) {
				// this is not a fake witness. the problem is that newPath can split into multiple paths and we can find a witness on a different one
		...
		Iterator<CGNode> callers = getCallers(path, this.callGraph);//new LinkedList<CGNode>();
		if (!callers.hasNext()) {
		...
		while (callers.hasNext()) {
			...
			if (Options.DEBUG) Util.Debug("trying caller " + caller + " " + (callCount++));// + " of " + callerList.size() + ";\n" + path);
			...
			while (sites.hasNext()) { // for each each caller
				...
				while (indexIter.hasNext()) { // for each (caller, callee) pair
					...
					Util.Assert(callBlk.getAllInstructions().get(callLine).equals(instr), 
								"calls " + instr + " and " + callBlk.getAllInstructions().get(callLine).equals(instr) + " don't match");
					...
					if (caller.equals(callee)) { // is this a recursive call?
						...
						if (newPath.foundWitness()) return true; // if dropping constraints led to fake witness, we're done
					...
					if (visitInvokeAsCaller((SSAInvokeInstruction) instr, callee, newPath)) { // execute call instruction
						...
						*/
						...
						if (!path.isCallRelevantToQuery((SSAInvokeInstruction) instr, callee, this.callGraph)) {
							Util.Debug("caller " + caller + " not relevant; skipping to beginning of method");
							newPath.setCurrentNode(caller);
							newPath.setCurrentBlock(caller.getIR().getControlFlowGraph().entry());
							newPath.setCurrentLineNum(-1);
							addPath(newPath);
							continue;
						} // else, caller is potentially important
						*/
						
						newPath.setCurrentLineNum(callLine - 1); // start from line before the call
						addPath(newPath); // path is feasible, add to paths to explore
					} else if (Options.DEBUG) Util.Debug("visiting caller yielded infeasible path; refuting");// else path infeasible, don't copy or add
				}
			}
		}
		return false;
	...
	List<IPathInfo> propagateBackwardToCallers(IPathInfo path) {
		...
		while (callers.hasNext()) {
			CGNode caller = callers.next();
			if (Options.DEBUG) Util.Debug("CALLER " + caller);
			...
			while (sites.hasNext()) { // for each each caller
				...
				while (indexIter.hasNext()) { // for each (caller, callee) pair
					...
					if (caller.equals(callee)) { // is this a recursive call?
						...
						newPath.skipCall((SSAInvokeInstruction) instr, this.callGraph, caller);
						//query.dropConstraintsProduceableInCall(instr, this.getCurrentNode(), callee);
						if (newPath.foundWitness()) return null; // if dropping constraints led to fake witness, we're done
						addPath(newPath); // add path and try next caller
						continue;
					}
					...
					if (WALACFGUtil.isFakeWorldClinit(caller, this.callGraph)) {
						...
						/*
						newPath.setCurrentNode(WALACFGUtil.getFakeWorldClinitNode(this.callGraph));
						if (handleFakeWorldClinit(newPath)) {
							newPath.declareFakeWitness(); // TODO: this is not a fake witness. the problem is that newPath can split into multiple paths and we can find a witness on a different one
							return null; // found witness in fakeWorldClinit; done
						}
						continue; // else, path refuted; try another
						*/
					...
					if (visitInvokeAsCaller((SSAInvokeInstruction) instr, callee, newPath)) { // execute call instruction
						if (Options.DEBUG) Util.Debug("done visiting caller\n" + newPath);
						if (newPath.foundWitness()) return null; // check for witness
						if (WALACFGUtil.isFakeWorldClinit(caller, this.callGraph)) {
							Util.Assert(false, "shouldn't be clinit here!");
							/*
							if (handleFakeWorldClinit(newPath)) {
	 ...
	 * selectPath() should return null if and only if there are no paths left to execute
	 * @return next path to execute, or null if there are none left
	 */
	@Override
	public IPathInfo selectPath() {
		if (!pathsToExplore.isEmpty()) return pathsToExplore.removeFirst();
		return null;
	}

	/**
	 * add a path to the paths to explore. override to change exploration strategy. default is DFS
	 * override to change exploration strategy. default is DFS
	 */
	@Override 
	public void addPath(IPathInfo path) {
		...
		// DEBUG for (IPathInfo info : pathsToExplore) Util.Assert(info.getPathId() != path.getPathId(), "path list already contains path " + path);
		if (Options.DEBUG) Util.Debug("adding path " + path.getPathId() + "X " + path.getCurrentNode() + " " + path.getCurrentBlock() + " line " + path.getCurrentLineNum());
		int stackSize = pathsToExplore.size() + 1;
		if (Options.DEBUG) Util.Debug("have " + stackSize + " paths to explore.");
	 ...
	 * explore all paths, but don't keep track of which conditions lead to paths being taken
	...
	public boolean executeBackwardsPathIntraprocedural(IPathInfo path) {
		...
		while (true) { // until path splits into multiple paths
			// if the current block has multiple predecessors, we must choose which one this path came from now in order to handle phi instructions correctly.
			...
			for (int i = instrs.size() - 1; i > -1; i--) {
				...
				if (i <= startLine) {
					...
					if (!visit(instr, path)) return false; // path is infeasible or has split
					else if (path.foundWitness()) return true; // found at witness after executing this instr - return now
				}
			}
			...
			if (preds.size() == 0) return true; // path has reached beginning of procedure
			else if (preds.size() == 1) {
				// optimization: for the single predecessor case, can execute next block directly (without any deep copying)
				...
				if (Options.DEBUG) Util.Debug("Forking execution: " + preds.size() + " choices.");
				boolean loop = false;
				if (WALACFGUtil.isLoopHead(currentBlock, ir) && path.seenLoopHead(currentBlock)) loop = true; // we've seen this loop head before

				// make copies of current path for each predecessor and add to list of paths to explore
				for (ISSABasicBlock blk : preds) {
					SSACFG.BasicBlock nextBlock = (SSACFG.BasicBlock) blk;
					if (loop && !WALACFGUtil.isLoopEscapeBlock(nextBlock, currentBlock, ir)) continue; // avoid infinite looping by skipping "loop taken" paths
					...
					if (loopHead != null) addLoopMergePlaceholder(loopHead);
	 ...
	 * @return - true if path is feasible after visiting instruction, false otherwise
	 */
	@Override
	public final boolean visit(SSAInstruction instr, IPathInfo info) {
		if (Options.DEBUG) Util.Debug(instr.toString());
	...
	/**
	 * wrapper to handle slightly unorthodox return behavior of IPathInfo's visit()
	 */
	boolean visitIPathInfoWrapper(SSAInstruction instr, IPathInfo info) {
		List<IPathInfo> caseSplits = info.visit(instr);
		if (caseSplits == IPathInfo.INFEASIBLE) return false; // infeasible
		for (IPathInfo path : caseSplits) addPath(path);
		return true;
	}
	
	/**
	 * wrapper to handle slightly unorthodox return behavior of IPathInfo's returnFromCall
	 */
	...
	/**
	 * visit invoke instruction as caller (i.e., we have executed the call and are now returning to the caller)
	 * note that the CGNode of info should be the caller node, NOT the callee
	...
	boolean visitInvokeAsCallee(SSAAbstractInvokeInstruction instr, IPathInfo info) {
		...
		//Util.Assert(!callees.isEmpty(), "Invoke instr " + instr + " doesn't resolve to any call sites from caller " + info.getCurrentNode());
		if (callees.isEmpty()) {
			Util.Debug("callees empty...skipping call");

			// TODO: hack! encode that size returns a value >= 0 and less than max collection size
		...
		if (callees.size() == 1) { // normal case
			...
			if (WALACFGUtil.isFakeWorldClinit(instr.getDeclaredTarget(), this.callGraph)) { // special handling for fakeWorldClinit
				info.pushCallStack((SSAInvokeInstruction) instr, callee);
				if (handleFakeWorldClinit(info)) {
					info.declareFakeWitness(); // TODO: this isn't actually a fake witness. the problem is that info can split and we can find a witness on a different path
			...
			return false; // don't want to continue executing instructions that occur before call in caller, so return false
			...
			for (CGNode callee : callees) { // consider case for each potential callee
				...
				info.skipCall((SSAInvokeInstruction) instr, this.callGraph, callee); // hack: don't consider dynamic dispatch
				/*
				IPathInfo copy = info.deepCopy();
				if (info.isCallRelevantToQuery(instr, callee, this.callGraph)) continue;
				if (visitCalleeWrapper(instr, callee, copy)) {
					added = true;
					addPath(copy); // don't want to continue executing instructions that occur before call in caller, so return false
				}
					// else, refuted by parameter binding
				}
				if (!added) return true; // no callees relevant; continue executing on current path
				*/
			}
			return true;
		}
	 ...
	 */
	boolean visitConditional(SSAConditionalBranchInstruction instr, IPathInfo info) { return true; }	
