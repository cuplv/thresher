...
/**
 * compute the set of {@ref CGNode's} from which control can flow to the current
 * block the srcs param to the constructor is the set of nodes from which flow
 * can originate this is a partially balanced parentheses problem, as we also
 * want to model flow that occurs from the callers of nodes in srcs
 * 
 * @author sam
 * 
 */
public class ReachingCallsAnalysis {
  ...
  public ReachingCallsAnalysis(ISupergraph<BasicBlockInContext<IExplodedBasicBlock>, CGNode> superGraph, CallGraph callgraph,
      Collection<CGNode> srcs) {
  ...
  private class ReachingCallsFlowFunctions implements IPartiallyBalancedFlowFunctions<BasicBlockInContext<IExplodedBasicBlock>> {
    ...
    public IUnaryFlowFunction getNormalFlowFunction(final BasicBlockInContext<IExplodedBasicBlock> src,
      ...
      if (src.getNode().equals(callgraph.getFakeRootNode()))
        return IdentityFlowFunction.identity();
  ...
  class ReachingCallsProblem implements
    ...
    /**
     * we don't need a merge function; the default unioning of tabulation works
     * fine
     */
