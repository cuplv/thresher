...
// PathQuery is the base class because it takes most of the info from the points-to query
public class CombinedPathAndPointsToQuery extends PathQuery {

	final PointsToQuery pointsToQuery;
	boolean fakeWitness = false;
	
	public CombinedPathAndPointsToQuery(PointsToQuery pointsToQuery) {//, Z3Context ctx) {
	...
	boolean visit(SSANewInstruction instr, CGNode node, SymbolTable tbl) {
		...
		if (pathVars.contains(local)) {
			...
			for (PointsToEdge edge : this.pointsToQuery.constraints) {
				Util.Debug("constraint pt edge " + edge);
				if (edge.getSource().equals(local)) { // does the points-to analysis hold a ref to this allocation site?
					substituteExpForVar(new SimplePathTerm(edge.getSink()), local);
					found = true;
				}
			} // if not found, no ref in points constraints; all a new() instr can do for us is resolve a constraint of the form "x != null"
			
			for (PointsToEdge edge : this.pointsToQuery.produced) {
				Util.Debug("produced pt edge " + edge);
				if (edge.getSource().equals(local)) { // does the points-to analysis hold a ref to this allocation site?
					substituteExpForVar(new SimplePathTerm(edge.getSink()), local);
					found = true;
				}
...
*/
	...
	/**
	 * if we are entering a call from a jump, we cannot do parameter binding directly, as we do normally
	 * instead, we must be a a bit more clever and use the dependency rules to help us make relevant bindings
	 */
	@Override
	public void enterCallFromJump(SSAInvokeInstruction instr, CGNode callee, IPathInfo currentPath) {
		...
		/*
		Set<DependencyRule> rulesProducedByCall = depRuleGenerator.getRulesForInstr(instr, currentPath.getCurrentNode());
		Util.Assert(rulesProducedByCall != null, "no rules for call " + instr + " from " + currentPath.getCurrentNode());
		for (DependencyRule rule : rulesProducedByCall) {
			Util.Debug("rule produced by call " + rule);
			PointsToEdge shown = rule.getShown();
			PointerVariable snk = shown.getSink();
			if (this.pathVars.contains(snk)) { // sub heap loc for its local pointer
				Util.Debug("enter call: subbing " + shown.getSource() + " for " + snk);
				substituteExpForVar(new SimplePathTerm(shown.getSource()), snk);
			}
		}
		*/
	}
	...
	List<IQuery> combinePathAndPointsToQueries(List<IQuery> pointsToQueries, List<IQuery> pathQueries) {
		...
		if (ptEmpty && pathEmpty) return IQuery.FEASIBLE;
		List<IQuery> combinedQuery = new LinkedList<IQuery>();
		if (!ptEmpty && !pathEmpty) {
			Util.Unimp("case split in path and points-to");
			// TODO: would need to mix in current query here as well, which would get messy...
			for (IQuery ptQuery : pointsToQueries) {
				for (IQuery pathQuery : pathQueries) {
					ptQuery.deepCopy();
					pathQuery.deepCopy();
					combinedQuery.add(new CombinedPathAndPointsToQuery((PointsToQuery) ptQuery.deepCopy(), 
																   (PathQuery) pathQuery.deepCopy()));
				}
			}
		} else if (!pathEmpty) {
			for (IQuery pathQuery : pathQueries) {
	...
	public boolean isCallRelevant(SSAInvokeInstruction instr, CGNode caller, CGNode callee, CallGraph cg) {
		return pointsToQuery.isCallRelevant(instr, caller, callee, cg) || this.doesCallWriteToHeapLocsInPathConstraints(instr, caller, callee, cg);
	}
	...
	public void removeLoopProduceableConstraints(SSACFG.BasicBlock loopHead, CGNode currentNode) {
		...
		// else, only need to drop path constraints; we're computing a fixed point over pt-constraints
		//super.removeLoopProduceableConstraints(loopHead, currentNode);
		// we only need to remove path constraints produceable in the loop... we don't want to remove pts-to constraints; we are computing a fixed point over those
		//dropConstraintsProuceableByRuleSet(Util.getRulesForLoop(loopHead, currentNode, depRuleGenerator, depRuleGenerator.getCallGraph()));
	...
	private void dropPathConstraintsWrittenInLoop(SSACFG.BasicBlock loopHead, CGNode node) {
		...
		Set<DependencyRule> rules = depRuleGenerator.getRulesForNode(node); // get all rules for node
		if (rules != null) {
			...
			dropConstraintsProuceableByRuleSet(loopRules); // remove all constraints produceable by one of these rules
		}
		...
		for (AtomicPathConstraint constraint : this.constraints) {
			keys.addAll(constraint.getPointerKeys());
			...
			for (SimplePathTerm term : terms) {
				if (term.getObject() != null && term.getFields() != null) {
					...
					if (pointedTo != null && pointedTo instanceof InstanceKey) {
						PointerKey key = hm.getPointerKeyForInstanceField((InstanceKey) pointedTo.getInstanceKey(), 
																		  cha.resolveField(term.getFirstField()));
	...
	public boolean contains(IQuery other) {
		if (!(other instanceof CombinedPathAndPointsToQuery)) return false;
	...
	public void dropConstraintsProduceableInCall(SSAInvokeInstruction instr, CGNode caller, CGNode callee) {
		...
		//Util.Assert(!this.foundWitness(), "dropping constraints led to fake witness!");
	}
	...
	void dropConstraintsProuceableByRuleSet(Set<DependencyRule> rules) {
		...
		for (AtomicPathConstraint constraint : this.constraints) {
			...
			for (DependencyRule rule : rules) {
				PointerVariable src = rule.getShown().getSource(); //snk = rule.getShown().getSink();
				//FieldReference field = rule.getShown().getFieldRef().getReference();
				if (vars.contains(src)) toRemove.add(constraint);
				if (rule.getShown().getFieldRef() != null && fields.contains(rule.getShown().getFieldRef().getReference())) toRemove.add(constraint);
	...
	void dropPathConstraintsProduceableByCall(SSAInvokeInstruction instr, CGNode caller, CGNode callee) {
		...
		//Set<DependencyRule> rulesProducedByCall = Util.getRulesProducableByCall(callee, depRuleGenerator.getCallGraph(), depRuleGenerator);
		...
		for (AtomicPathConstraint dropMe : toDrop) removeConstraint(dropMe); //constraints.remove(dropMe);
	}
	...
	boolean doesCallWriteToHeapLocsInPathConstraints(SSAInvokeInstruction instr, CGNode caller, CGNode callee, CallGraph cg) {
		...
		for (int i = 0; i < instr.getNumberOfParameters(); i++) {
			if (!tbl.isConstant(instr.getUse(i))) {
				...
				if (this.pathVars.contains(arg)) return true; // constraints contain a non-constant param that's passed to this function; possibly relevant
			}
		}
	...
	boolean substituteActualsForFormals(SSAInvokeInstruction instr, CGNode callerMethod, CGNode calleeMethod, 
		...
		for (int i = 0; i < instr.getNumberOfParameters(); i++) {
			...
			if (callerSymbolTable.isIntegerConstant(use)) {
				...
				// check for formal in pts-to constraints
				for (PointsToEdge edge : pointsToQuery.constraints) {
					Util.Debug(edge.getSource() + " eq " + formal);
					if (edge.getSource().equals(formal)) {
			...
			else if (callerSymbolTable.isStringConstant(use)) actual = SimplePathTerm.NON_NULL; // the only modeling we do of strings is that they are non-nul
			else if (callerSymbolTable.isConstant(use)) Util.Unimp("other kinds of constants"); // TODO: support other constants
	...
	*/
	...
	public void removeAllLocalConstraints() {
		this.removeAllLocalPathConstraints(); // IMPORTANT! must do this first, otherwise we lose the local pts-to info!
		pointsToQuery.removeAllLocalConstraints();
	}
	
	/**
	 * take advantage of pts-to information to sub out locals for their heap value, if known
	 */
	public void removeAllLocalPathConstraints() {
		...
		for (AtomicPathConstraint constraint : toRemove) removeConstraint(constraint);//constraints.remove(constraint);
	}

	@Override
	public boolean addConstraintFromBranchPoint(IBranchPoint point, boolean trueBranchFeasible) {
		return this.addConstraintFromBranchPointAndCheckForNull(point, trueBranchFeasible) && pointsToQuery.addConstraintFromBranchPoint(point, trueBranchFeasible);
	}
	...
	public boolean addConstraintFromBranchPointAndCheckForNull(IBranchPoint point, boolean trueBranchFeasible) {
		...
		if (tbl.isConstant(instruction.getUse(0)) && tbl.isConstant(instruction.getUse(1))) {
			...
			if (constraint == null) return true; // null return means its a string comparison or something else we don't support. bail outc	
			
			// check for null comparison against something that's already in the pts-to constraints
			if (constraint.getLhs() instanceof SimplePathTerm && constraint.getRhs() instanceof SimplePathTerm) {
				...
				if (lhs.isIntegerConstant() && lhs.getIntegerConstant() == 0) comparedToNull = rhs.getObject();
				else if (rhs.isIntegerConstant() && rhs.getIntegerConstant() == 0) comparedToNull = lhs.getObject();
				if (comparedToNull != null) {
					ConditionalBranchInstruction.Operator op = constraint.getOp();
					// this is a null comparison; see if this appears in the pts-to constraints
					for (PointsToEdge edge : pointsToQuery.produced) {
	...
	/**
	 * @return - set of *all* pointer keys associated with constraint, including the ones related according to the points-to analysis
	 */
		...
		for (SimplePathTerm term : terms) {
			if (term.getObject() != null && term.getFields() != null) {
				...
				if (pointedTo != null && pointedTo instanceof InstanceKey) {
					PointerKey key = hm.getPointerKeyForInstanceField((InstanceKey) pointedTo.getInstanceKey(), 
																	  cha.resolveField(term.getFirstField()));
					if (key != null) keysForConstraint.add(key);
				}
			}
		}
	...
	private Map<Constraint,Set<CGNode>> getModifiersForQueryHelper() {
		...
		for (AtomicPathConstraint constraint : this.constraints) {
			...
			//addClassInitsForConstraintFields(constraint, nodes); // add class init if it may write to the constraint
			for (PointerKey key : getPointerKeysForPathConstraint(constraint)) {
				Set<CGNode> modRefNodes = reversedModRef.get(key);
				if (modRefNodes == null) continue; // this can happen when var is the this param for a class with no fields
	...
	/**
	 * add the class init for each field to the set of relevant nodes
	 * need to do this because class inits can write to fields by
	 * writing their default values. this implicit write is not
	 * reflected in the normal mod/ref analysis
	 * @param constraint
	 * @param nodes
	 */
	/*
	private void addClassInitsForConstraintFields(AtomicPathConstraint constraint, Set<CGNode> nodes) {
		...
		for (PointerKey key :constraint.getPointerKeys()) {
			...
			IMethod clit = fieldKey.getDeclaringClass().getClassInitializer();
			Util.Debug("classInit is " + clit);
	...
	*/
	...
	public boolean containsConstraint(Constraint constraint) {
		if (constraint instanceof PointsToEdge) return pointsToQuery.containsConstraint(constraint);
		return super.containsConstraint(constraint);
	}
	
	@Override
	public List<DependencyRule> getWitnessList() { return pointsToQuery.getWitnessList(); }
	
	/*
	@Override
	public int compareTo(Object other) {
		if (!(other instanceof CombinedPathAndPointsToQuery)) Util.Unimp("comparing with different kind of query");
		CombinedPathAndPointsToQuery otherQuery = (CombinedPathAndPointsToQuery) other;
		int ptComparison = pointsToQuery.compareTo(otherQuery.pointsToQuery);
		if (ptComparison != 0) return ptComparison;
		int pathComparison = super.compareTo((PathQuery) otherQuery);
