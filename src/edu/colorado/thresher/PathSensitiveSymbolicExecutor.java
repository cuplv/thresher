...
/**
 * interprocedural symbolic executor that precisely tracks correlations between conditional branch outcomes and paths taken
...
public class PathSensitiveSymbolicExecutor extends BasicSymbolicExecutor {
	...
	/** 
	 * @param preds - predecessors of current execution block
	...
	void initializeSplitPaths(LinkedList<IPathInfo> splitPaths, Collection<ISSABasicBlock> preds, IPathInfo path) {
		...
		if (preds.size() < 2) { // sometimes, we need to add the exceptional predecessors here
		...
		if (Options.DEBUG) Util.Assert(preds.size() > 1, "can't create split path lists with less than 2 preds!");
		
		
		
		//Util.Debug("initializing split paths; currentBlock is " + path.getCurrentBlock());
		...
		if (Options.DEBUG) Util.Assert(!splitPaths.isEmpty(), "should never return empty split paths list!");
	}	

	/**
	 * run path through all of the instructions in the current block (backwards)
	 * @param path - current path
	 * @param splitPaths - list of splits the current path has taken so far
	 * @param loopHead - null if the current block is not a loop head, the loop head otherwise
	 * @return - true if path is feasible after executing all instructions, false otherwise
	 */
		...
		for (int i = instrs.size() - 1; i > -1; i--) {
			...
			if (i <= startLine) {
				...
				if (instr instanceof SSAPhiInstruction) {
					// found a phi node; need to do path splitting early in order to decide which value is assigned on which path
					//if (splitPaths.isEmpty()) {
					int phiIndex = instr.getNumberOfUses() - 1;
					if (phiIndexPathsMap.isEmpty()) {
						if (splitPaths.isEmpty()) initializeSplitPaths(splitPaths, preds, path);
					...
					for (; phiIndex >= 0; phiIndex--) {
						...
						for (IPathInfo choice : choicesForIndex) {
							if (!isLoopHead || !WALACFGUtil.isLoopEscapeBlock(choice.getCurrentBlock(), loopHead, ir)) {
								if (Options.DEBUG) Util.Debug("correlating phi index " + phiIndex + " with block " + choice.getCurrentBlock());
					...
					// "normal" case. this should precede executing any phi instructions, so we don't need to consider the case splits here
					path.setCurrentLineNum(i - 1);
					if (!visit(instr, path)) {
						//if (Options.CHECK_ASSERTS) Util.Assert(!path.isFeasible() || split, "path should be marked infeasible here or we should have a split");
						return false;
					}
				}
			}
		}
		if (!phiIndexPathsMap.isEmpty()) { // push the content of the phi index map back into split paths
			splitPaths.clear();
	 ...
	 * after calling this, should check if returned true, and if so, should check for witness
	 * if false is returned, the next path in the list should be selected and executed
	 * @return true if procedure boundary is reached / witness is found, false if path splits or becomes infeasible
	 */
	@Override
	public boolean executeBackwardsPathIntraprocedural(IPathInfo path) {
		if (Options.CHECK_ASSERTS) {
			...
			// true return => path feasible && (found witness || at procedure boundary || no preds for current block)
			// can have no preds for current block but not be at an entry due to exceptional control flow (we only get non-exceptional control flow)
			Util.Post(!result || (path.isFeasible() && (path.foundWitness() || path.getCurrentBlock().isEntryBlock() || 
								  path.getCurrentNode().getIR().getControlFlowGraph().getNormalPredecessors(path.getCurrentBlock()).isEmpty())), 
					  "feasible?" + path.isFeasible() + " blk " + path.getCurrentBlock() + " witness " + path.foundWitness() + " path " + path);
			...
			Util.Post(result || (!path.isFeasible() || split),
					   " post failure on  path " + path + " blk " + path.getCurrentBlock() + "IR " + path.getCurrentNode().getIR());
			split = false;
			return result;
		}
	...
	private boolean executeBackwardsPathIntraproceduralImpl(final IPathInfo path) {
		...
		for (;;) {
			...
			if (!splitPaths.isEmpty()) {
				...
				for (IPathInfo choice : splitPaths) {
					if (choice.isFeasible() &&
						...
						(!loopHead || (WALACFGUtil.isLoopEscapeBlock(choice.getCurrentBlock(), currentBlock, ir) || 
								!choice.seenLoopHead(currentBlock)))) {
							addPath(choice);
					}
				}
			...
			if (!preds.isEmpty() && WALACFGUtil.isLoopHead(currentBlock, ir)) {
				// odd situation; in a loop head, but don't know what the conditional controlling the loop is. this may happen either because we entered the 
				// loop head from a callee that precedes the conditional, or because there's not conditional controlling the loop (it's explicitly infinite)
				
				// if the loop is explicitly infinite, we should refute if our path entered the loop from outside, but continue if we started in the loop or
				// entered via a callee
				
				// TODO: support refuting infinite loops that we entered from outside
				if (Options.DEBUG) Util.Debug("in loop, but don't know conditional");
				if (Options.DEBUG) Util.Assert(splitPaths.isEmpty(), "expecting empty split paths here!");
				...
				// else, this loop is not explicitly infinite; we just don't know the conditional yet.
				return handleLoopHead(path, currentBlock.getLastInstruction());
			}
			
			/*
			// if we reach this point the current block should NOT be a loop head (unless we're leaving a proc whose first block is a loop head)
			Util.Assert(preds.isEmpty() || !WALACFGUtil.isLoopHead(currentBlock, ir), 
					    "not expecting loop head here! BLK " + currentBlock + "IR:\n" + ir + "preds: " + preds.size());
					    */
			
			// have executed all instructions in currentBlock. proceed to predecessors, if there are any			
			if (preds.isEmpty()) { // no preds;
				if (currentBlock.isEntryBlock()) return true; // at proc boundary; return true
				Util.Debug("refuted by thrown exception");
				path.refute();
				return false; // else, means we're in a block with only exceptional predecessors
			}
				...
				for (IPathInfo newPath : splitPaths) { 
					...
					if (possibleLoopHead != null) addLoopMergePlaceholder(possibleLoopHead);
					addPath(newPath);
				}
				if (Options.CHECK_ASSERTS) split = true;
				return false; // path has split
	...
	void visitCallInLoopHead(SSAInvokeInstruction instr, IPathInfo path) {
		// ignore call and drop constraints it may produce. this is to avoid getting lost in the call if it is complicated; we can afford to explore an expensive call once,
		...
		for (CGNode callee : callees) {
			path.skipCall(instr, callGraph, callee); // skipCall drops constraints the call might produce
		}
	}
	
	/**
	 * special case for loop heads that are more than one block in length. keep executing until we reach the block connecting the back edge
	 */
	boolean executeAllInstructionsInLoopHeadSequence(IPathInfo info, LinkedList<IPathInfo> splitPaths) {
		...
		Set<IPathInfo> cases= new TreeSet<IPathInfo>(); // list to handle case splits in straight-line code (i.e. many applicable rules)
		cases.add(info);
		// run path through all of the instructions in the loop head sequence, storing any case splits in splitPaths
		...
		for (;;) {
			...
			for (int i = instrs.size() - 1; i > -1; i--) {
				...
				if (i <= startLine) {
					if (instr instanceof SSAInvokeInstruction) {
						...
						// "normal" case
						if (Options.DEBUG) Util.Assert(!(instr instanceof SSAConditionalBranchInstruction), "should never be executing conditionals here!");
						if (Options.DEBUG) Util.Assert(splitPaths.isEmpty(), "shouldn't have split yet!");
						List<IPathInfo> toAdd = new LinkedList<IPathInfo>();
						List<IPathInfo> toRemove = new LinkedList<IPathInfo>();
						for (IPathInfo path : cases) {
			...
			if (preds.size() == 1) {
				...
				if (Options.DEBUG) Util.Assert(phiIndexMap == null, "phiIndex map should not be init!");
				...
				if (Options.DEBUG)Util.Assert(preds.size() > 1, "loop should split at some point!");
				//Util.Assert(initSplitPaths, "split paths not initialized!");
				splitPaths.clear(); 
				if (phiIndexMap == null) { // phiIndexMap was never initialized. need to split into a case for each pred
					if (Options.DEBUG) Util.Assert(splitPaths.isEmpty(), "split paths should be empty here!");
					if (cases.isEmpty()) {
						info.refute();
						return false;
					}
					if (Options.DEBUG) Util.Assert(cases.size() == 1, "too many cases " + cases.size());
					for (IPathInfo _case : cases) initializeSplitPaths(splitPaths, preds, _case);
				...
				for (List<IPathInfo> list : lists) splitPaths.addAll(list);
				return !splitPaths.isEmpty();
	 ...
	 */
	/*
	boolean executeAllInstructionsInLoopHeadSequence(IPathInfo path, LinkedList<IPathInfo> splitPaths) {
		// run path through all of the instructions in the loop head sequence, storing any case splits in splitPaths
		// do NOT add any paths to the path manager; add them to caseSplits instead.
		final IR ir = path.getCurrentNode().getIR();
		final SSACFG cfg = ir.getControlFlowGraph();
		...
		for (;;) {
				...
				if (i <= startLine) {
					if (instr instanceof SSAInvokeInstruction) {
						...
						for (int key : phiIndexMap.keySet()) {
							...
							List<IPathInfo> toAdd  = new LinkedList<IPathInfo>();
							for (IPathInfo choice : values) {
								List<IPathInfo> cases = visitPhi((SSAPhiInstruction) instr, choice, key);
								if (cases == IPathInfo.INFEASIBLE) {
									Util.Debug("phi visit made path infeasible");
									toRemove.add(choice); // phi visit made path infeasible
								} else if (!cases.isEmpty()) {
									Util.Debug("case plitting on phi");
									toAdd.addAll(cases);
								}
							}
							values.addAll(toAdd);
						...
						for (List<IPathInfo> list : lists) {
							newSplitPaths.addAll(list);
						}
						
						for (IPathInfo choice : toRemove) newSplitPaths.remove(choice);
					} else {
						...
						List<IPathInfo> cases = path.visit(instr);
						if (cases == IPathInfo.INFEASIBLE) return false; // infeasible
						Util.Assert(cases == IPathInfo.FEASIBLE, "not expecting case splits here! " + cases.size());
						for (IPathInfo split : cases) {
							executeAllInstructionsInLoopHeadSequence(split, splitPaths);
						}
						for (IPathInfo split : cases) splitPaths.add(split);
			...
			if (preds.size() == 1) {
				currentBlock = (SSACFG.BasicBlock) preds.iterator().next();
				for (IPathInfo choice : splitPaths) choice.setCurrentBlock(currentBlock);
	...
	*/
	...
	List<IPathInfo> visitPhi(SSAPhiInstruction instr, IPathInfo info, int phiIndex) {
		if (Options.DEBUG)Util.Pre(phiIndex >= 0, "phi index should be non-negative!");
		if (Options.DEBUG)Util.Assert(instr.getNumberOfDefs() == 1, "only expecting one def in phi; found " + instr);
		if (Options.DEBUG)Util.Assert(phiIndex < instr.getNumberOfUses(), "index " + phiIndex + " out of bounds for # of uses " + instr.getNumberOfUses());
	...
	public void addLoopMergePlaceholder(SSACFG.BasicBlock loopHeadToMerge) {
		...
		//Util.Assert(!pathsToExplore.contains(mergeIndicator), "already have loop merge indicator for " + loopHeadToMerge);
		if (!pathsToExplore.contains(mergeIndicator)) {
			if (Options.DEBUG) Util.Debug("adding loop merge indicator for block " + loopHeadToMerge);
