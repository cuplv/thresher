...
public class PathTermWithBinOp implements PathTerm {
	...
	public boolean symbContains(PathTerm other) {
		...
		if (other instanceof SimplePathTerm) return false;
	...
	*/
	...
	public int compareTo(Object other) {
		if (other instanceof SimplePathTerm) return -1;
	...
	public int evaluate() {
		...
		switch (this.binOp) {
			case ADD: return lhsVal + rhsVal;
			case SUB: return lhsVal - rhsVal;
			case MUL: return lhsVal * rhsVal;
			case DIV: return lhsVal / rhsVal;
			case REM: return lhsVal % rhsVal;
			default: 
	...
	public Z3AST toZ3AST(Z3Context ctx, boolean boolVar) {
		...
		switch (this.binOp) {
			case ADD: return ctx.mkAdd(binOpLHS, binOpRHS);
			case SUB: return ctx.mkSub(binOpLHS, binOpRHS);
			case MUL: return ctx.mkMul(binOpLHS, binOpRHS);
			case DIV: return ctx.mkDiv(binOpLHS, binOpRHS);
			case AND:
				Util.Unimp("bw and");
			...
			case REM: return ctx.mkRem(binOpLHS, binOpRHS);
			default: Util.Unimp("unsupported bin op " + binOp);
		}
		return null;
	}
	...
	@Override
	public boolean isHeapLocation() { return false; } // this is never just a heap location because it contains a bin op
	...
	public PointerVariable containsField(FieldReference field) {
		...
		return rhs.containsField(field);
	}
	*/
