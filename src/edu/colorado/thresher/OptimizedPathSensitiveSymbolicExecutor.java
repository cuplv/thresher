...
/**
 * path-sensitive symbolic executor that uses "branch points" optimization to merge paths at conditional branches / loop heads
 * @author sam
 */
public class OptimizedPathSensitiveSymbolicExecutor extends PathSensitiveSymbolicExecutor {
	...
	boolean handleLoopHead(IPathInfo info, SSAInstruction instr) {
			...
			if (Options.SUBSUMPTION_CHECK_AT_SUMMARIES) {
				...
				for (IPathInfo path : seen) {
					if (info.containsQuery(path)) { // have we seen a simpler path?
						if (Options.DEBUG) Util.Debug("refuted by loop summary: " + path + " simpler than " + info);
						info.refute();
						return false;
					} else if (path.containsQuery(info)) { // no; is this one simpler?
						if (Options.DEBUG) Util.Debug("this path simpler than one we've seen; swapping");
						toRemove.add(path);
					}
				}
				...
				if (!seen.add(info)) {
					if (Options.DEBUG) Util.Debug("refuted by loop summary");
					info.refute();
					return false;
		...
		// TODO: hack to deal with stupid labeled break/continue control flow. in reality, we want point != null <==> info.seenLoopHead()
		if (info.seenLoopHead(currentBlock) && (point != null)) { 
			//info.removeLoopProduceableConstraints(currentBlock);
			if (Options.DEBUG) Util.Debug("already seen loop head");
			...
			if (!point.addPathToLoopHead(info)) {
				if (Options.DEBUG) Util.Debug("already seen this path... stopping execution");
				//Thread.dumpStack();
				executeAllNonPhiInstructionsInCurrentBlock(info);
				if (Options.CHECK_ASSERTS) split = true;
				return false; // already seen this path at the loop head; don't keep executing
			} 
			this.logger.logPathWithRelevantLoop(info);
			// else, haven't seen this path before; keep executing
			if (Options.DEBUG) Util.Debug("haven't seen this particular path at the loop head before - continuing");
			seenLoopHead = true;
		} 
		if (Options.DEBUG) Util.Debug("haven't seen loop head before");
		if (!seenLoopHead) {
			if (Options.DEBUG) Util.Debug("adding loop merge indicator for block " + currentBlock);
			addLoopMergePlaceholder(currentBlock);
		}
		LinkedList<IPathInfo> splitPaths = new LinkedList<IPathInfo>(); // list to hold list of split paths if split occurs before end of block
		...
		if (Options.DEBUG) { Util.Assert(pathCount == this.pathsToExplore.size(), "should not have added or removed any paths here!");
		...
		for (IPathInfo choice : splitPaths) {
			if (choice.isFeasible()) {
				if (WALACFGUtil.isLoopEscapeBlock(choice.getCurrentBlock(), currentBlock, ir)) {
					if (Options.DEBUG) Util.Debug("loop escape block " + choice.getCurrentBlock());
					//seenEscape = true;
					// don't execute loop escape path until we've seen all paths through the loop; stash it in branch point
					if (!seenLoopHead) point.addPathToLoopHead(choice);
				} else {
	...
	boolean visitConditional(SSAConditionalBranchInstruction instr, IPathInfo info) {
		...
		if (Options.CHECK_ASSERTS) split = true;
		return false; // never want to continue execution after a conditional branch instruction - only after branch point is merged
	}
	
	/**
	 * remove and return a path from paths to explore. override to change exploration strategy. default is DFS
	...
	public IPathInfo selectPath() {
		if (!pathsToExplore.isEmpty()) { 
			...
			if (path.isLoopMergeIndicator()) { // special case for merging loops; we must do so eagerly or we will drop too many constraints
				if (Options.DEBUG) Util.Debug("forcing loop merge");
				if (!branchPointStack.isEmpty()) return mergeBranchPointForLoopHead(path.getCurrentBlock()); // do merge
				else return this.selectPath(); // else do nothing
				//else Util.Assert(false, "couldn't find branch point for loop head " + path.getCurrentBlock());
			}
			if (Options.DEBUG) Util.Assert(path != null, "path should never be null here!");
			return path; // "normal" case; return path on top of stack
		} else {
			if (!branchPointStack.isEmpty()) return mergeNextBranchPoint(); // no paths left in stack; merge branch points, if there are any
			else return null; // no paths left in stack and no branch points left to merge 
		}
	}
	
	/**
	 * returns null if we can't get a non-dummy path. should never return loop merge indicator or dummy path
	 * @return
	 */
	public IPathInfo selectNonDummyPath() {
		IPathInfo path = null;
		if (pathsToExplore.peek().isDummy() && branchPointStack.peek().isDummy()) { // are there any non-dummy paths or branch pts?
			path = null;
			...
			if (path.isDummy()) {
				if (Options.DEBUG) Util.Assert(this.branchPointStack.peek().isDummy(), "expect null branch point stack here!");
				this.pathsToExplore.addFirst(path); // replace path and return null; we're done
				if (Options.DEBUG) Util.Debug("replacing dummy path on stack");
				path = null;
			}
			// else, can simply return path
		} else if (!branchPointStack.peek().isDummy()) { // no paths left to explore; need to merge branch point
		...
		*/
		if (Options.DEBUG) Util.Post(path == null || (!path.isDummy() && !path.isLoopMergeIndicator()), "bad path " + path);
		return path;
	}

	/**
	 * 
	 * @param truePaths - list of feasible paths skipping the loop 
	 * @param falsePaths - list of feasible paths entering the loop 
	 * @return
	 */
	...
	public IPathInfo mergeLoop(Set<IPathInfo> truePaths, Set<IPathInfo> falsePaths, SSACFG.BasicBlock loopHeadBlock) {
		...
		// commented out because not necessarily true... if we start in a loop, one of the lists will be empty
		//Util.Assert(!truePaths.isEmpty() && !falsePaths.isEmpty(), "both path lists should be non empty!");
		...
		if (Options.DEBUG) Util.Debug("seen " + truePaths.size() + " paths at loop head");
		for (IPathInfo path : truePaths) {
			if (Options.DEBUG) Util.Debug("PATH " + path);
		}
		
		for (IPathInfo info : truePaths) {
			info.setCurrentBlock(loopHeadBlock);
			info.setCurrentLineNum(loopHeadBlock.getAllInstructions().size() - 2); // start before the branch instr
			...
			/*else {
				for (IPathInfo path : cases) {
					path.removeSeenLoopHead(loopHeadBlock); // forget that we saw this loop head; needed for nested loops
				}
			}*/
		}
		...
		for (IPathInfo info : truePaths) {
			if (!info.containsLoopProduceableConstraints(loopHeadBlock)) {
				info.removeSeenLoopHead(loopHeadBlock); // forget that we saw this loop head; needed for nested loops
				addPath(info);
			}
		}
		...
		// TODO: if the LHS is a local and the right side is a local allocation site that's not in this node, it's infeasible...
		...
		return selectPath();
		// at this point, base contains the path constraints that are common to all paths in the loop
		/*
		// for each query, replace constraints that could be produced in the loop with their preconditions
		for (IPathInfo info : truePaths) {
			info.intersectQuery(base); // drop path constraints by intersecting with common constraints
			info.removeLoopProduceableConstraints(loopHeadBlock);
			mergedPaths.add(info); // merge paths; removing loop produceable constraints may have caused previously distinct paths to become the same
		}
		
		// run each path through the runup to the loop
		for (IPathInfo info : mergedPaths) {
			info.setCurrentBlock(loopHeadBlock);
			info.setCurrentLineNum(loopHeadBlock.getAllInstructions().size() - 2); // start before the branch instr
			List<IPathInfo> cases = executeAllInstructionsInLoopHeadBlock(info);
			Util.Assert(cases.isEmpty(), "not expecting case split here!");
			info.removeSeenLoopHead(loopHeadBlock); // forget that we saw this loop head; needed for nested loops
			Util.Debug("Added loop path " + info);
			addPath(info);
		}
		Util.Debug("done merging loop; added " + mergedPaths.size() + " paths.");
		
		// TODO: DEBUG only
		IPathInfo somePath = mergedPaths.iterator().next();
		IR ir = somePath.getCurrentNode().getIR();
		SSACFG.BasicBlock escapeBlock = somePath.getCurrentBlock();
		
		// loop apex is the block that is the sink of the back edge. I use the term "loop head block" to refer to blocks in the loop head between the loop apex
		// and the conditional branch instruction that actually controls the loop. the loop apex is a loop head block, but not all loop head blocks are the apex
		Collection<ISSABasicBlock> succs = ir.getControlFlowGraph().getNormalSuccessors(escapeBlock);
		Util.Assert(succs.size() == 1, "should be exactly one successor");
		SSACFG.BasicBlock loopApex = (SSACFG.BasicBlock) succs.iterator().next(); // escape block should have one successor, and it should be the loop apex
		Util.Assert(WALACFGUtil.isLoopEscapeBlock(escapeBlock, loopApex, ir), 
				"should be in loop escape block here! in block " + somePath.getCurrentBlock() + " instead.");
		// TODO: end debug
		return selectPath(); // return the path from the top of the path stack
		*/
	}
	/*
	public IPathInfo mergeLoop(Set<IPathInfo> truePaths, Set<IPathInfo> falsePaths, SSACFG.BasicBlock loopHeadBlock) {
		Util.Log("merging loop");
		// commented out because not necessarily true... if we start in a loop, one of the lists will be empty
		//Util.Assert(!truePaths.isEmpty() && !falsePaths.isEmpty(), "both path lists should be non empty!");
		truePaths.addAll(falsePaths);  
		...
		boolean first = true;
		IPathInfo base = null;
		// take intersection of all queries; that is, "drop" constraints that may be produced in the loop
		for (IPathInfo info : truePaths) {
			Util.Debug("loop intersect path " + info);
			Util.Assert(info.isFeasible(), "infeasible path at loop head!");
			if (first) {
				base = info.deepCopy();
				first = false;
			} else {
		...
		IR ir = base.getCurrentNode().getIR();
		// execute all instructions in loop head, make sure loop escape block will be taken
		base.setCurrentBlock(loopHeadBlock);
		base.setCurrentLineNum(loopHeadBlock.getAllInstructions().size() - 2); // start before the branch instr
		List<IPathInfo> cases = executeAllInstructionsInLoopHeadBlock(base);
		
		SSACFG.BasicBlock escapeBlock = base.getCurrentBlock();
		// loop apex is the block that is the sink of the back edge. I use the term "loop head block" to refer to blocks in the loop head between the loop apex
		// and the conditional branch instruction that actually controls the loop. the loop apex is a loop head block, but not all loop head blocks are the apex
		Collection<ISSABasicBlock> succs = ir.getControlFlowGraph().getNormalSuccessors(escapeBlock);
		Util.Assert(succs.size() == 1, "should be exactly one successor");
		SSACFG.BasicBlock loopApex = (SSACFG.BasicBlock) succs.iterator().next(); // escape block should have one successor, and it should be the loop apex
		Util.Assert(WALACFGUtil.isLoopEscapeBlock(escapeBlock, loopApex, ir), 
				"should be in loop escape block here! in block " + base.getCurrentBlock() + " instead.");
		
		for (IPathInfo info : cases) {
			info.removeSeenLoopHead(loopHeadBlock);
			addPath(info);
		}
		base.removeSeenLoopHead(loopHeadBlock); // forget that we saw this loop head; needed for nested loops
		return base;
	}
	*/
	...
	List<IPathInfo> executeAllInstructionsInLoopHeadBlock(IPathInfo info) {
		...
		// make sure this isn't an explicitly infinite loop (no branching). otherwise, we would spin around the loop below forever
		if (WALACFGUtil.isExplicitlyInfiniteLoop(currentBlock, ir)) { // is this loop explicitly infinite?
		...
		for (;;) {
			for (int i = instrs.size() - 1; i > -1; i--) {
				...
				if (i <= startLine) {
					...
					if (instr instanceof SSAPhiInstruction) {
						int phiIndex = preds.size() - 1; // the loop escape block is always the last choice in the phi
						// we are leaving the loop, so choose the last item on the list of possible phi's (always corresponds to the escape block)
						List<IPathInfo> toAdd = new LinkedList<IPathInfo>(), toRemove = new LinkedList<IPathInfo>();
						for (IPathInfo path : caseSplits) {
							...
							if (cases == IPathInfo.INFEASIBLE) toRemove.add(path);
							else toAdd.addAll(cases);
						}
						...
						if (Options.DEBUG) Util.Assert(!(instr instanceof SSAConditionalBranchInstruction), "should never execute branch instr's here!");
						// "normal" case
						List<IPathInfo> toAdd = new LinkedList<IPathInfo>(), toRemove = new LinkedList<IPathInfo>();
						for (IPathInfo path : caseSplits) {
							path.setCurrentLineNum(i - 1);
							List<IPathInfo> splits = path.visit(instr);
							if (splits == IPathInfo.INFEASIBLE) toRemove.add(path); // infeasible
							else toAdd.addAll(splits);
						}
						caseSplits.addAll(toAdd);
						caseSplits.removeAll(toRemove);
			...
			if (preds.size() == 1) { // keep executing straight-line code
				...
				if (Options.DEBUG) Util.Assert(!preds.isEmpty(), "loop should split eventually!");
				//Util.Assert(!caseSplits.isEmpty(), "no paths left to execute!");
				if (caseSplits.isEmpty()) return IPathInfo.INFEASIBLE;
				...
				for (ISSABasicBlock pred : preds) {
					...
					if (WALACFGUtil.isLoopEscapeBlock(nextBlock, currentBlock, ir)) {
						for (IPathInfo path : caseSplits) {
							//Util.Debug("selecting loop escape block " + nextBlock + " for " + path.getPathId());
				...
				// TODO: is this the right thing to do? or a non-termination bug? this is triggered by nested for each loops
	...
	/**
	 * select the branch point on the top of the stack and merge it. if it does not corresponds to the loop head contained in mergeBlock, 
	 * replace the loop placeholder 
	 * @param mergeBlock - block containing the loop head we need to merge
	...
	public IPathInfo mergeBranchPointForLoopHead(SSACFG.BasicBlock mergeBlock) {
		...
		if (!point.getBlock().equals(mergeBlock)) {
			// this is the not the loop we are supposed to merge; replace the placeholder
			addLoopMergePlaceholder(mergeBlock); 
		} // else, this was the loop head we were supposed to merge; can merge and then continue execution as normal
	...
	public IPathInfo mergeNextBranchPoint() {
		if (Options.DEBUG) Util.Pre(!branchPointStack.isEmpty(), "Trying to merge with no mergeable branch points!");
		IBranchPoint point = branchPointStack.pop();
		branchPointMap.remove(point.getBranchPointKey()); // delete this branch point so we don't double merge
		return mergeBranchPoint(point);
	}
	...
	public IPathInfo mergeBranchPoint(IBranchPoint point) {
		...
		for (IPathInfo info : truePaths) {
			if (Options.DEBUG) Util.Assert(info.isFeasible(), "infeasible path " + info);
			if (!info.isFeasible()) toRemove.add(info); 
		}
		for (IPathInfo removeMe : toRemove) {
			boolean removed = truePaths.remove(removeMe);
			if (Options.DEBUG) Util.Assert(removed, "can't remove path!");
		}	
		toRemove.clear();
		...
		// at this point, an empty true/false path set means no feasible paths on true/false branch
		...
		if (point.isLoopHead()) {
			...
			return mergeLoop(truePaths, falsePaths, point.getBlock()); // loops are a special case here...may need to drop constraints
		}
		...
		if (truePathsEmpty && falsePathsEmpty) { // no paths are feasible
			...
			if (Options.DEBUG) Util.Debug("no false paths feasible");
			for (IPathInfo info : truePaths) {
				// add path constraints to each path in truePaths and continue
				if (info.addConstraintFromBranchPoint(point, true)) toAdd.add(info); // path is feasible after adding constraint 
			}
			...
			for (IPathInfo falsePath : falsePaths) {
				for (IPathInfo truePath : truePaths) {
					...
					if (falsePath.equals(truePath)) {
						// we can arrive at this path regardless of if we take true path or false path; we can continue on a single path with no constraints added
						// problem: if two paths have the same constraints, but one of the paths has seen a loop head, we want to mark the other one as having 
						// seen it as well. this is an efficiency question; if the constraints are the same whether we make one pass or two passes through the 
						// loop, we would prefer not to explore the loop another time
						if (falsePath.loopsSeen() > truePath.loopsSeen()) toAdd.add(falsePath); // continue execution on the path that has seen more loop heads
						else toAdd.add(truePath);
						toRemove.add(truePath); // we could also add falsePath here; they're the same
					}
				}
			}
			// remove paths that are in both sets from each set; we've already added them to the path manager 
			for (IPathInfo removeMe : toRemove) {
				boolean removed = truePaths.remove(removeMe);
				if (Options.DEBUG) Util.Assert(removed, "couldn't remove " + removeMe);
			}
			for (IPathInfo removeMe : toRemove) {
				boolean removed = falsePaths.remove(removeMe);
				if (Options.DEBUG) Util.Assert(removed, "couldn't remove " + removeMe);
			}
			// now, each path that remains must have the appropriate constraint added to it
			for (IPathInfo falsePath : falsePaths) {
				if (falsePath.addConstraintFromBranchPoint(point, false)) toAdd.add(falsePath); // add path constraint and path to path manager
			}
			for (IPathInfo truePath : truePaths) {
				if (truePath.addConstraintFromBranchPoint(point, true)) toAdd.add(truePath); // add path constraint and path to path manager
			}
		}
		...
		return selectPath();
				/*
		if (!pathsToExplore.isEmpty()){
			IPathInfo next = pathsToExplore.removeFirst(); // return path on the top of the stack
			Util.Assert(!next.isLoopMergeIndicator(), "loop merge indicator is the next path! " + next);
			return next;
		}
		return null; // no paths left to explore
		*/
	}
	
	/**
	 * special case for do...while loops, which can have non-phi instructions in the loop head
	 * to run through the loop, we must execute these instructions (but not the phi's, since they are always executed upon exiting the loop)
	 */
	boolean executeAllNonPhiInstructionsInCurrentBlock(IPathInfo path) {
		...
		for (int i = instrs.size() - 1; i > -1; i--) {
			SSAInstruction instr = instrs.get(i);
			if (i <= startLine) {
				if (instr instanceof SSAPhiInstruction) break; // phi's are always the first instr's in a block
				path.setCurrentLineNum(i - 1);
				if (!visit(instr, path)) return false;
			}
		}
		return true;
	}
	...
	/**
	 * bubble this branch point up the stack until it dominates another branchInstr in the stack
	 * @param newPoint - IBranchPoint to bubble
	 */
	void pushBranchStack(IBranchPoint newPoint) { 
		...
		for (int i = branchPointStack.size() - 1; i > -1; i--) {
			...
			if (newPoint.getIR().equals(point.getIR())) { // make sure IR's are equal, else dominator check may fail
				if (WALACFGUtil.isDominatedBy(blk, newBlk, newPoint.getIR())) {
					//Util.Debug(blk + "is dominated by " + newBlk + "adding " + newPoint + " at " + i);
					if (i == branchPointStack.size() - 1) branchPointStack.addLast(newPoint);
					else branchPointStack.add(i + 1, newPoint);
					return;
				}
	...
	/**
	 * symbolic execution in fakeWorldClinit (WALA's model of the Java class initializers) requires special handling. 
	 * we do not attempt to model the order that static initializers are invoked in, so we ask if each class initializer
	 * can produce our constraints, and execute it if so
	 */
	@Override
	boolean handleFakeWorldClinit(IPathInfo path) {
		...
		//path.removeAllLocalConstraintsFromQuery(); // can't reason about params to fakeWorldClinit!
		...
		while (path != null) {
			if (!path.getCurrentNode().equals(fakeWorldClinitNode)) {
				...
				if (path.foundWitness()) return true;
				if (hitProcBoundary) {
					if (Options.DEBUG) Util.Debug("hit proc boundary");
					//if (path.getCurrentNode().equals(fakeWorldClinitNode)) continue; // at proc boundary for fakeWorldClinit; witness not produced
					if (!path.isCallStackEmpty()) {
						// leaving callee and returning to called method
						if (path.returnFromCall()) continue;//addPath(path); // add path if it is feasible
						//continue;
					}
				}
			} else {
				...
				for (SSAInstruction instr : instrs) { // for each class initializer
					...
					if (!(instr instanceof SSAInvokeInstruction)) { // this is an instruction, but not an entrypoint
					...
					if (Options.DEBUG) Util.Assert(targets.size() == 1, "Not expecting dynamic dispatch in class inits; everything should be static");
				...
				if (path.foundWitness()) return true; // last constraint was a static field pointing to it's default value
			}
	...
	/*
	//@Override
	boolean handleFakeWoerldClinit(IPathInfo path) {
		Util.Debug("trying to find witness in fakeWorldClinit");
		CGNode fakeWorldClinitNode = path.getCurrentNode();
		Util.Assert(fakeWorldClinitNode.equals(WALACFGUtil.getFakeWorldClinitNode(callGraph)), 
				fakeWorldClinitNode + "should be fakeWorldClinit!");
		// TODO: rather than iterating over the class initializers, be a little more selective?
		...
		addPathAndBranchPlaceholders();
		Set<SSAInstruction> alreadyTried = new HashSet<SSAInstruction>();
		for (;;) { // allow class inits to be executed in any order, keep making progress towards witness
			boolean progress = false;
			for (SSAInstruction instr : instrs) { // for each class initializer
				if (alreadyTried.contains(instr)) continue;
				if (instr == null) continue;
				if (!(instr instanceof SSAInvokeInstruction)) {
					if (!super.visitIPathInfoWrapper(instr, path)) return false;
					continue;
				}
				//Util.Assert(instr instanceof SSAInvokeInstruction, "non-invoke instr " + instr + " in class inits");
				SSAInvokeInstruction instruction = (SSAInvokeInstruction) instr;
				Set<CGNode> targets = callGraph.getPossibleTargets(fakeWorldClinitNode, instruction.getCallSite());
				Util.Assert(targets.size() == 1, "Not expecting dynamic dispatch in class inits!");
				CGNode classInitializer = targets.iterator().next();
				
				// ask the current path if this class initializer can produce any part of its query
				if (path.isCallRelevantToQuery(instruction, classInitializer, callGraph)) {
					Util.Debug("Trying class initializer " + instr);
					alreadyTried.add(instruction);
					// yes; push the class initializer on the call stack and execute it
					path.pushCallStack(instruction, classInitializer);
					while (true) {
						// until we're done exploring this call
						if (this.executeBackwardsPathIntraprocedural(path)) {
							if (path.foundWitness()) {
								Util.Debug("found witness while exploring class inits! done.");
								return true; // found witness
							}
							// else, finished executing call, but did not produce witness
							if (!path.returnFromCall()) {
								cleanupPathAndBranchPlaceholders();
								return false; // return from the class init or current method
							}
							if (!path.isCallStackEmpty()) {
								Util.Debug("call stack not empty; continuing to execute");
								continue; // not done with the class init; keep executing
							}
							progress = true;
							break; // and pick a new one
						} else {
							path = selectNonDummyPath();
							if (path == null) {
								cleanupPathAndBranchPlaceholders();
							...
							// else, the path is fine; go ahead and execute
							continue;
						}
					}
				} else Util.Debug("class init " + classInitializer + " not relevant");
			} // end for each instructon
			if (!progress) {
				path.removeAllLocalConstraintsFromQuery();
				if (path.foundWitness()) return true;
				path = selectNonDummyPath();
				if (path == null) break; // if there's no progress, stop executing class initializers. the witness is not going to be produced in the loop
			}
		} // end for
		// else, still haven't produced...
...
*/
	/**
	 * symbolic execution in fakeRootMethod (WALA's model of the Java class initializers) requires special handling. 
	 * we do not attempt to model the order that static initializers are invoked in, so we ask if each class initializer
	 * can produce our constraints, and execute it if so
	 */
	...
	boolean handleFakeRootMethod(IPathInfo path, CGNode entrypoint) {
		if (Options.DEBUG) Util.Pre(path.getCurrentNode().equals(this.callGraph.getFakeRootNode()), "current node should be fakeRootNode at this point!");
		if (Options.DEBUG) Util.Debug("trying to find witness in fakeRootMethod");
		
		//path.removeAllLocalConstraintsFromQuery(); // can't reason about params to entrypoints
		if (path.foundWitness()) return true;
		...
		while (path != null) {
			if (!path.getCurrentNode().equals(fakeRootMethod)) {
				...
				if (hitProcBoundary) {
					if (!path.isCallStackEmpty()) {
						// leaving callee and returning to called method
						if (path.returnFromCall()) continue; // keep executing path if it is feasible //addPath(path); // add path if it is feasible
					}
				}
			} else {
				...
				for (SSAInstruction instr : instrs) { // for each entrypoint
					if (Options.DEBUG) Util.Debug("trying entrypoint instr " + instr);
					if (!alreadyTried.add(instr)) continue; // TODO: UNSOUND. we need to allow arbitrary, but somehow bounded re-execution of class entrypoints
					if (instr == null) continue;
					if (!(instr instanceof SSAInvokeInstruction)) { // this is an instruction, but not an entrypoint
						if (!super.visitIPathInfoWrapper(instr, path)) break; // refuted; try next path
						if (path.foundWitness()) return true;
					...
					for (CGNode target : targets) {
						if (target.equals(fakeWorldClinit)) continue; // class inits are handled separately
						else if (target.equals(entrypoint)) continue; // don't want to execute our same entrypoint again
						// ask the current path if this class initializer can produce any part of its query
						if (path.isCallRelevantToQuery(instruction, target, callGraph)) {
							if (Options.DEBUG) Util.Debug("Trying entrypoint " + instr);
							IPathInfo copy = path.deepCopy();
					...
					if (split) break; // added all targets; proceed to executing paths
				} 
				if (!split && path.isFeasible()) toTryInFakeWorldClinit.add(path); // tried all entrypoints and no witness found here; try fakeWorldClinit
			}
			// else, path refuted or split; pick next one
			path = selectNonDummyPath();
		}
		
		if (Options.DEBUG) Util.Debug("witness not produced in fakeRootMethod! proceeding to fakeWorldClinit");
		cleanupPathAndBranchPlaceholders();
		for (IPathInfo info : toTryInFakeWorldClinit) {
			...
			if (handleFakeWorldClinit(info)) return true;
		} 
	...
	boolean handleFakeRootMethod(IPathInfo path, CGNode entrypoint) {
		...
		Util.Assert(path.getCurrentNode().equals(this.callGraph.getFakeRootNode()), "current node should be fakeRootNode at this point!");
		...
		Set<SSAInstruction> alreadyTried = new HashSet<SSAInstruction>();
		CGNode fakeWorldClinit = WALACFGUtil.getFakeWorldClinitNode(callGraph);
		for (;;) { // allow entrypoints to be executed in any order, keep making progress towards witness
			boolean progress = false;
			for (SSAInstruction instr : instrs) { // for each class initializer
				if (alreadyTried.contains(instr)) continue;
				if (instr == null) continue;
				...
				Set<CGNode> targets = callGraph.getPossibleTargets(fakeRootMethod, instruction.getCallSite());
				for (CGNode target : targets) {
				//Util.Assert(targets.size() == 1, "Not expecting dynamic dispatch in fakeRootMethod!");
				CGNode classInitializer = target;
				if (classInitializer.equals(fakeWorldClinit)) continue; // class inits are handled separately
				else if (classInitializer.equals(entrypoint)) continue; // don't want to execute our same entrypoint again
				// ask the current path if this class initializer can produce any part of its query
				if (path.isCallRelevantToQuery(instruction, classInitializer, callGraph)) {
					...
					while (true) {
						...
						if (this.executeBackwardsPathIntraprocedural(path)) {
							...
							progress = true;
							break; // and pick a new one
						} else { // visit false; pick new path
							path = selectNonDummyPath();
							if (path == null) {
								cleanupPathAndBranchPlaceholders();
								return false;
			...
			if (!progress) {
				toTryInFakeWorldClinit.add(path);
				path = selectNonDummyPath();
				if (path == null) break; // if there's no progress, stop executing fakeRootMethod. the witness is not going to be produced here
			}
		} // end for
	...
	*/
	...
	public void addPathAndBranchPlaceholders() {
		if (Options.DEBUG) Util.Debug("adding path and branch placeholders");
		/* DEBUG
		for (IPathInfo path : this.pathsToExplore) {
			Util.Assert(!path.isDummy(), "should not contain dummy paths if this is called!");
		}
		
		for (IBranchPoint point : branchPointStack) {
			Util.Assert(!point.isDummy(), "should not contain dummy points if this is called!");
		}
		*/ 
	...
	public void cleanupPathAndBranchPlaceholders() {
		if (Options.DEBUG) Util.Debug("cleaning up path and branch placeholders");
		...
		if (!Options.PIECEWISE_EXECUTION) { // the piecwise symbolic executor multi-stacks these; can't do this check
			for (IPathInfo path : this.pathsToExplore) {
