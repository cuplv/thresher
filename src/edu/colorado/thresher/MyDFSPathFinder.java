...
public class MyDFSPathFinder<T> extends Stack<T> {
  ...
  /**
   * Construct a depth-first enumerator starting with a particular node in a
   * directed graph.
   * 
   * @param G
  ...
  /**
   * Construct a depth-first enumerator across the (possibly improper) subset of
   * nodes reachable from the nodes in the given enumeration.
   * 
   * @param nodes
      ...
      for (Iterator<? extends T> it = getPendingChildren(stackTop); it.hasNext();) {
        ...
        if (stackTop != null) {
          if (notAllowed != null && notAllowed.contains(G.getNumber(child))) {
            ...
            // edge is second of pair of edges that should be excluded; don't
