 ...
 * contains program point information, a query, and utility methods for interacting with
 * symbolic executor and query
 * @author sam
 *
 */
public class IPathInfo { //implements Comparable {
	...
	/** 
	 * create and return "dummy" IPathInfo (can be used as placeholder on branch stack)
	 */
	public static IPathInfo makeDummyPath() { return new IPathInfo(); }
	
	public static IPathInfo makeMergeIndicator(SSACFG.BasicBlock toMerge) { return new IPathInfo(toMerge); } 
	...
	private IPathInfo(CGNode currentNode, SSACFG.BasicBlock currentBlock, SSACFG.BasicBlock lastBlock, int currentLineNum, LinkedList<IStackFrame> callStack, 
			 Set<Pair<CGNode,SSACFG.BasicBlock>> loopHeadSet, IQuery query, IQuery initialQuery, PiecewiseGraph piecewiseGraph) {
		Util.Pre(currentBlock != null, "current block should not be null!");
		this.pathId = pathIdCounter++;
		this.currentNode = currentNode;
		this.currentBlock = currentBlock;
	...
	public IPathInfo deepCopy() {
		...
		return new IPathInfo(this.currentNode, this.currentBlock, this.lastBlock, this.currentLineNum, Util.deepCopyStackList(this.callStack), 
				 			 Util.deepCopySet(loopHeadSet), query.deepCopy(), this.initialQuery, this.piecewiseGraph.deepCopy());
	}
	
	/**
	 * copy the current path, but replace the current query with a newQuery
	 * NOTE: newQuery should be deep copied before it is passed
	 */
	public IPathInfo deepCopyWithQuery(IQuery newQuery) {
		return new IPathInfo(this.currentNode, this.currentBlock, this.lastBlock, this.currentLineNum, Util.deepCopyStackList(this.callStack), 
							 Util.deepCopySet(loopHeadSet), newQuery, this.initialQuery, this.piecewiseGraph);
	}
	...
	public boolean simulateQueryReturnFromCall(SSAInvokeInstruction instruction, CGNode callee) {
		//Util.Debug("simulating return from callee " + callee + " via call instr " + instruction);
		...
		if (query instanceof CombinedPathAndPointsToQuery) {
			...
			if (instr.hasDef()) {
				...
				if (combined.pathVars.contains((returnValue))) {
					...
					combined.constraints.add(new AtomicPathConstraint(new SimplePathTerm(returnValue), new SimplePathTerm(MAX_COLLECTION_SIZE),
							   ConditionalBranchInstruction.Operator.LT));
	...
	/**
	 * pop the call stack and restore the state from the caller. this should only be called when returning from a callee (i.e. the call stack is empty)
	...
	public boolean returnFromCall() {
		...
		if (Options.DEBUG) Util.Debug("returning from " + callee + " to " + frame.getCGNode());
		//MethodReference calleeMethod = callee.getMethod().getReference();
		
		// cleanup: forget which loop heads we saw in the callee, in case we come back to the callee later
		...
		for (Pair<CGNode,SSACFG.BasicBlock> pair: loopHeadSet) {
			if (pair.fst.equals(callee)) toRemove.add(pair);
		}
		/*
		for (SSACFG.BasicBlock loopHead : loopHeadSet) {
			if (loopHead.getMethod().getReference().equals(calleeMethod)) toRemove.add(loopHead);
		}
		for (SSACFG.BasicBlock removeMe : toRemove) loopHeadSet.remove(removeMe);
		*/
		...
		// we should never have case splits here because the arguments to the callee were already known before we entered it
		Util.Post(caseSplits == IQuery.INFEASIBLE || caseSplits.isEmpty(), "Shouldn't have case splits after leaving callee!");
		return caseSplits == IQuery.INFEASIBLE ? false : true;
	}
	
	/**
	 * special case; should be called ONLY when returning from a call and our call stack is empty (i.e. branching backwards to a new method
	...
	public List<IPathInfo> returnFromCall(SSAInvokeInstruction instr, CGNode callee) {
		...
		// cleanup: forget which loop heads we have seen. otherwise, if we come back to this method later, we will neglect to execute loops we've already seen
		loopHeadSet.clear();
	...
	/**
	 * push the current state onto the call stack and set the state to the callee state
	 * @param callInstr - the call instr in the caller that forced this call stack push
	 * @param callee - CGNode for the callee method
	 */
	void pushCallStack(SSAInvokeInstruction callInstr, CGNode callee) {
		...
		//if (this.currentNode.getMethod().toString().equals(callee.getMethod().toString())) {
		if (this.currentNode.equals(callee)) {
			//Util.Print("Recursion detected; bailing out! method is " + callee.getMethod().getName().toString());
			Util.Unimp("recursion");
		}
			
		//Util.Debug("pushing " + this.currentNode + " onto call stack and entering " + callee);
		...
		//Util.Assert(addContextualConstraints(currentNode), "refuted by contextual constraints!");
	...
	/**
	 * remove a loop head from the loops we have seen already... important for nested loops
	 * @param loopHead - a loop head we have just merged
	 */
	...
	public int loopsSeen() { return loopHeadSet.size(); }
	
	/**
	 * change this path's query to be the intersection of the current query and other's query
	 * @param other - path whose query we will intersect
	 */
	...
	List<IPathInfo> handleQueryCaseSplitReturn(List<IQuery> caseSplits) {
		if (caseSplits == IQuery.INFEASIBLE) return IPathInfo.INFEASIBLE;
		else if (caseSplits.equals(IQuery.FEASIBLE)) return IPathInfo.FEASIBLE;
		else {
			List<IPathInfo> paths = new LinkedList<IPathInfo>();
			for (IQuery query: caseSplits) {
				if (query != initialQuery) {
				//if (!query.contains(initialQuery)) { // don't want to allow inital query to be re-added; should be considered during separate execution
	...
	public List<IPathInfo> visitPhi(SSAPhiInstruction instr, int phiIndex) {
		List<IQuery> caseSplits = query.visitPhi(instr, phiIndex, this); //query.visitPhi(instr, phiIndex, this);
		return handleQueryCaseSplitReturn(caseSplits);
	}
	...
	/**
	 * alter constraints to reflect that we are entering callee. note that currentNode is still the caller node here
	 * @param instr - invoke instruction making call to callee
	 * @param callee - method being called
	 * @return - list of paths to be executed (in addition to this one) if case split occurs during visit
	 */
	private List<IPathInfo> enterCall(SSAInvokeInstruction instr, CallGraph cg, CGNode callee, boolean skip) {
		...
		//if (skip || !loopHeadSet.isEmpty() || callee.getIR() == null || calleeName.contains("equals") || calleeName.contains("hashCode") ||
		if (skip || callee.getIR() == null || calleeName.contains("equals") || calleeName.contains("hashCode") || calleeName.contains("indexOf") ||
			calleeName.contains("Iterator") || !isCallRelevantToQuery(instr, callee, cg)) { // if this call is relevant
			// TODO: hack! just want to avoid executing equals() / hashCode() because it's a deathtrap and unlikely to lead to refutation
			...
			if (Options.DEBUG) Util.Debug("skipping recursive call " + callee.getMethod().toString() + " and dropping produced constraints");
			...
			/*
			// DEBUG - sanity check for possible mutual recursion; is our current node already on the call stack?
			for (int i = 0; i < callStack.size(); i++) {
				//if (callStack.get(i).getCGNode().equals(currentNode)) { // is this node already on the call stack?
				if (callStack.get(i).getCGNode().equals(callee)) { // is this node already on the call stack?				
					// yes; check for mutual recursion
					//if (isMutuallyRecursive(currentNode, i)) {
					if (isMutuallyRecursive(callee, i)) {
						Util.Debug("possible mutual recursion on node " + currentNode + " call stack depth " + callStack.size() + " skipping + dropping constraints");
						query.dropConstraintsProduceableInCall(instr, this.getCurrentNode(), callee);
						return IPathInfo.FEASIBLE;
					}
				}
			...
			*/
			...
			if (callStack.size() > Options.MAX_CALLSTACK_DEPTH) { // is our call stack too deep?
				if (Options.DEBUG) Util.Debug("skipping ordinary call " + callee + " due to call stack depth and dropping produced constraints");
				query.dropConstraintsProduceableInCall(instr, this.getCurrentNode(), callee);
				return IPathInfo.FEASIBLE;
			}
		}
		
		if (Options.DEBUG) Util.Debug("entering call " + callee.getMethod().toString() + " from " + currentNode.getMethod().toString());
	...
	/**
	 * inspect call stack and determine if callee is part of a mutually recursive sequence
	 * @param callee - node we want to push on the call stack, but are scared is mutually recursive. 
	 * @param offendingIndex - index of previous occurence of callee in call stack
	 * @return true if callee is part of mutually recursive sequence, false otherwise
	 */
	/*
	boolean isMutuallyRecursive(CGNode node, int offendingIndex) {
		Util.Pre(callStack.get(offendingIndex).getCGNode().equals(node), "callstack should contain callee at the specified index!");
		//Util.Assert(offendingIndex != 0, "non-mutual recursion should have been caught by the regular recursion case!");// return false; // no sequence leading up to the first call...not confirmed mutual recursion
		// let the callee be X. say that we suspect X is part of a mutually recursive sequence X C1 C2 ... X
		// we first record the sequence C1 C2 ... (top of call stack), then check backwards from the first occurence of X in the call stack to see if the sequence is matched
		LinkedList<CGNode> sequence = new LinkedList<CGNode>(); // the sequence of calls from the first occurrence of X to the top of the call stack
		for (int i = offendingIndex + 1; i < callStack.size(); i++) {
			sequence.push(callStack.get(i).getCGNode());
		}
		Util.Assert(!sequence.isEmpty(), "non-mutual recursion should have been caught by the regular recursion case! " + Util.printCollection(callStack));

		// see if the sequence of calls leading up to this occurrence of X matches the previous sequence
		int i = offendingIndex - 1;
		while (!sequence.isEmpty()) {
			if (i == -1) break;
			if (!sequence.pop().equals(callStack.get(i).getCGNode())) return false;
			i--;
		}
	...
	*/
	...
	boolean isMutuallyRecursive(CGNode callee, int offendingIndex) {
		...
		// let the callee be X. say that we suspect X is part of a mutually recursive sequence X C1 C2 ... X
		// we first record the sequence C1 C2 ... X, then check backwards from the first occurence of X in the call stack to see if the sequence is matched
		LinkedList<CGNode> sequence = new LinkedList<CGNode>(); // the sequence of calls from the first occurrence of X to the top of the call stack
		for (int i = offendingIndex + 1; i < callStack.size(); i++) {
			sequence.push(callStack.get(i).getCGNode());
		}
		Util.Assert(!sequence.isEmpty(), "non-mutual recursion should have been caught by the regular recursion case!");
		if (offendingIndex == 0) return false; // no sequence leading up to the first call...not confirmed mutual recursion
		// see if the sequence of calls leading up to this occurrence of X matches the previous sequence
	...
	/**
	 * search backwards in CFG until we find a branch instr; push it onto the branch stack
	 */
	/*
	void findAndPushNextBranchInstruction(IR ir) {
		//final IR ir = this.getCurrentNode().getIR();
		SSACFG.BasicBlock currentBlock = this.getCurrentBlock();
		SSACFG.BasicBlock lastBlock = this.getLastBlock(); 
		int startLine = this.getCurrentLineNum();
		
		while (true) { // until we find a branch instr or reach the end of the procedure
			...
			for (int i = instrs.size() - 1; i > -1; i--) {
				...
				if (i <= startLine) {
					if (instr instanceof SSAConditionalBranchInstruction) {
						IBranchPoint point = IBranchPoint.makeBranchPoint((SSAConditionalBranchInstruction) instr, i, currentBlock, ir);
						this.pushBranchStack(point, lastBlock);
						return;
					}
				}
			}
			// don't look at instructions in current block; we will never find the branch instruction we are looking for there
			Collection<ISSABasicBlock> preds = ir.getControlFlowGraph().getNormalPredecessors(currentBlock);
			if (preds.size() == 0) return; // path has reached beginning of procedure
			else if (preds.size() == 1) {
				// optimization: for the single predecessor case, can go to next block directly
			...
			} else return; // the path has split; will find the branch instr's corresponding to this split when we reach it during execution 
		}
	}
	*/
	
	public boolean isCallStackEmpty() { return callStack.isEmpty(); }
	
	public static int getPathIdCounter() { return pathIdCounter; }

	public CGNode getCurrentNode() { return currentNode; }

	public void setCurrentNode(CGNode currentNode) {
		this.currentNode = currentNode; 
		addContextualConstraints(currentNode); // TODO: is this information reflected elsewhere?
	}

	public SSACFG.BasicBlock getCurrentBlock() { return currentBlock; }

	/** 
	 * set currentBlock and store old currentBlock as lastBlock
	 */
	public void setCurrentBlock(SSACFG.BasicBlock currentBlock) { 
		if (Options.DEBUG) Util.Pre(currentBlock != null, "can't set block to null!");
		this.lastBlock = this.currentBlock; 
		this.currentBlock = currentBlock;
	}
	
	public SSACFG.BasicBlock getLastBlock() { return lastBlock; }

	public int getCurrentLineNum() { return currentLineNum; }

	public void setCurrentLineNum(int currentLineNum) { this.currentLineNum = currentLineNum; }

	public int getPathId() { return pathId; }
	
	public boolean foundWitness() { return query.foundWitness(); }
	
	public boolean isCallRelevantToQuery(SSAInvokeInstruction instr, CGNode callee, CallGraph cg) { return query.isCallRelevant(instr, this.currentNode, callee, cg); }
	
	public void declareFakeWitness() {
		if (Options.DEBUG) Util.Debug("declaring fake witness");
	...
	public static void resetPathIdCounter() { pathIdCounter = 1; }
	...
	public boolean isFeasible() {
		...
		return query != null && // null query means that this is a dummy path (i.e. branch placeholder)
			   query.isFeasible(); 
	}
	...
	/**
	 * call to force refutation of path even when query is not explicitly contradictory
	 * for example, if we have some mechanism for summaries, we should call refute() when
	 * a path is refuted by a summary
	 */
	public void refute() {
	...
	public void setAtBranchPoint(boolean bool) { this.atBranchPoint = bool; }
	
	public boolean atBranchPoint() { return atBranchPoint; }
	
	public void initializeStaticFieldsToDefaultValues() { query.intializeStaticFieldsToDefaultValues(); }
	
	public List<DependencyRule> getWitnessList() { return query.getWitnessList(); }
	...
	public boolean equals(Object other) {
		if (!(other instanceof IPathInfo)) return false;
		...
		if (query == null && otherInfo.query == null) {
			...
			return false;
		} else if ( (query == null) != (otherInfo.query == null) ) return false; // one query is null and the other isn't
		if (this.pathId == otherInfo.getPathId()) return true; // optimization; save cost of comparing queries when id's are equal
	...
	public int compareTo(Object other) {
		if (!(other instanceof IPathInfo)) Util.Unimp("comparing pathInfo to non-pathInfo");
		IPathInfo otherPath = (IPathInfo) other;
		if (query == null && otherPath.query == null) return 0;
		if (query == null) return -1;
		if (otherPath.query == null) return 1;
		return query.compareTo(otherPath.query);
	}
	*/
	
	@Override 
	public String toString() {
		if (query == null) return "DUMMY";//this.getPathId() + "";
	...
	static class PiecewiseGraph {
		...
		public boolean addSeen(IPathInfo path, CGNode caller) {
			...
			if (seen) addTraversedEdge(path.getCurrentNode(), caller);
			else addSkippedEdge(path.getCurrentNode(), caller);
			return seen;
		}
