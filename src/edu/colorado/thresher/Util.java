...
public class Util {
	...
	public static void Debug(String s) {
		if (Options.DEBUG) System.out.println(s);
	}
	...
	*/
	...
	public static DependencyRule makeUnconditionalDependencyRule(SSAInstruction instr, Object lhs, Object rhs, PointerStatement.EdgeType type, 
																 int lineId, int lineNum, CGNode node) {
		...
		if (lhsPointer != null & rhsPointer != null) {
			...
			return new DependencyRule(edge, stmt, new TreeSet<PointsToEdge>(), node, (SSACFG.BasicBlock) node.getIR().getBasicBlockForInstruction(instr));
		}
		return null;
	}
	...
	/**
	 * gives all dependency rules generated both directly by a call and transitively by its callees
	 * @param call - the base call from which to generate rules
	 * @return - set of dependency rules that give all rules generated by call and its callees
	 */
	public static Set<DependencyRule> getRulesProducableByCall(CGNode call, CallGraph cg, AbstractDependencyRuleGenerator depRuleGenerator) {
		Util.Unimp("don't call this");
		return depRuleGenerator.getRulesForNodeAndCallees(call);
		/*
		Set<CGNode> toExplore = new HashSet<CGNode>(), explored = new HashSet<CGNode>();
		Set<DependencyRule> rules = new TreeSet<DependencyRule>();
		toExplore.add(call);
		while (!toExplore.isEmpty()) {
			call = toExplore.iterator().next();
			explored.add(call);
			toExplore.remove(call);
			depRuleGenerator.generateRulesForNode(call);
			Set<DependencyRule> rulesForCall = depRuleGenerator.getRulesForNode(call);
			if (rulesForCall != null) rules.addAll(rulesForCall);
			else Util.Debug("no rules for node " + call);
			Iterator<CallSiteReference> sites =  call.iterateCallSites();
			while (sites.hasNext()) {
				CallSiteReference site = sites.next();
				Set<CGNode> targets = cg.getPossibleTargets(call, site);
				for (CGNode target : targets) {
					if (!explored.contains(target)) toExplore.add(target);
				}
			}
		}
		return rules;
	...
	private static boolean writesKeyLocally(CGNode node, ArrayContentsKey key, HeapModel hm, HeapGraph hg, ClassHierarchy cha) {
		...
		Set<Object> preds = iteratorToSet(hg.getPredNodes(key)); // get array references that point at this ArrayContentsKey
		IR ir = node.getIR();
		...
		for (int i = 0; i < instrs.length; i++) {
			...
			if (instr instanceof SSAArrayStoreInstruction) {
				...
				while (succs.hasNext()) {
					// if the ArrayContentsKey and refs pointed to by the local match, we have a possible write
					if (preds.contains(succs.next())) return true;
				}
	...
	public static Set<DependencyRule> getProducersForEdge(PointsToEdge edge, AbstractDependencyRuleGenerator depRuleGenerator) {
		...
		if (snk == null) {
			// TODO: support this
			// trying to witness something pointing to a constant... return empty rule set for now
			return rules;
		...
		*/
		...
		} else if (edge.getField() instanceof ArrayContentsKey) array = true;
		else Util.Assert(edge.getField() == null || edge.getField() instanceof StaticFieldKey, "odd field " + edge.getField());
		...
		if (src instanceof LocalPointerKey) { // this edge has a local LHS
			...
			for (i = 0; i < instrs.length; i++) {
				...
				if (instr.hasDef() && instr.getDef() == lpk.getValueNumber()) {
					...
					for (DependencyRule rule : instrRules) {
						if (rule.getShown().equals(edge)) rules.add(rule); // this rule produces our edge
					}
					break; // because of SSA, this must be the only statement that produces our edge
				}
			}
			if (rules.isEmpty()) { // it's possible that the edge is produced by a phi, which aren't in the regular instruction array
			...
			// sets to keep track of value nums of local pointers. these are overapproximate because they don't contain any information about
			// methods they belong to, but it doesn't seem worth it to save that information
			...
			if (!staticSrc) {
				...
				while (iter0.hasNext()) { // find locals that point at the src, since we must write to it through some local ptr
			...
			while (iter1.hasNext()) { // find locals that point at the snk, since we need a local ptr to write it to snk
			...
			if (staticField) {
				...
				for (CGNode node : snkMethods) {
					...
					for (i = 0; i < instrs.length; i++) {
						...
						if (instr instanceof SSAPutInstruction) {
							SSAPutInstruction put = (SSAPutInstruction) instr;
							if (put.isStatic() && snkNums.contains(put.getUse(0)) &&
								put.getDeclaredField().equals(key.getField().getReference())) {
	...
	public static Set<CGNode> getPotentialProducersForEdgeAndGenerateRules(PointsToEdge edge, HeapGraph hg, AbstractDependencyRuleGenerator depRuleGenerator, CallGraph cg) {
		Util.Unimp("don't call this");
		 Util.Debug("getting rules relevant to " + edge);
		 ...
		 if (edge.getSource().toString().contains("fakeRootMethod")) srcMethods.add(cg.getFakeRootNode());
		 if (edge.getSink().toString().contains("fakeRootMethod")) snkMethods.add(cg.getFakeRootNode()); 
		 ...
		 for (CGNode node : snkMethods) {
			 ...
			 //if (DEBUG) System.err.println("generated rules for node " + count + " of " + snkMethods.size() + "; have " + aDepRuleGenerator.numRules() + " rules");
		}
		 return snkMethods;
	 }
	 
	 public static List<CGNode> getMethodsProducingEdge(PointsToEdge edge, HeapGraph hg, AbstractDependencyRuleGenerator depRuleGenerator, CallGraph cg) {
	 ...
	 /**
	  * @param edge - edge to be produced
	  * @return all dependency rules that could possibly produce that edge
	  */
	 public static Set<DependencyRule> getRulesProducingEdge(PointsToEdge edge, HeapGraph hg, AbstractDependencyRuleGenerator depRuleGenerator, CallGraph cg) {
		 Util.Unimp("don't call this");
		 Set<CGNode> sharedMethods = getPotentialProducersForEdgeAndGenerateRules(edge, hg, depRuleGenerator, cg);
	 ...
	 /**
	  * 
	  * @param edge - the edge to be checked for produceability in loop
	  * @param loopHead - head of the loop we are looking at
	  * @param node - CGNode containing the loop we are currently in
	  * @return set of dependency rules in loop that can produce edge. these dependency rules are modified so *only* their non-loop preconditions are retained
	  */
	 public static List<DependencyRule> getLoopProducersForEdge(PointsToEdge edge, SSACFG.BasicBlock loopHead, CGNode node, HeapGraph hg, 
		...
		Set<DependencyRule> loopRules = getRulesForLoop(loopHead, node, depRuleGenerator, cg); 
		 ...
		 for (DependencyRule rule : loopRules) {
			 if (!rule.getShown().equals(edge)) continue; // this rule does not produce our edge
			 ...
			 if (WALACFGUtil.isInLoopBody(ruleBlk, loopHead, rule.getNode().getIR())) {
				 ...
				 // TODO: need to do mini fixed-point computation here; keep adding preconditions as long as at least one cannot be produced in the loop
				 
				 
				 // see if preconditions are also produceable in the loop body. if they are not, do not add them to our constraints
				 for (PointsToEdge pre : rule.getToShow()) {
					 if (!isEdgeProduceableByLoop(pre, loopHead, node, hg, depRuleGenerator, cg)) toAdd.add(pre);
					 else {
						 Util.Debug("dropped node is " + pre.getSource().getNode());
						 Util.Debug("current node is " + node);
						 Util.Assert(pre.getSource().getNode().equals(node), "nodes " + pre.getSource().getNode() + " and " + node + " don't match");
						 //noRule = true;
						 //break;
				 ...
				 // make dummy rule that replaces edge with non-loop-produceable preconditions
				 ...
				 for (DependencyRule producer : rulesProducingEdge) {
					 if (producer.equalExceptForStatement(dummy)) {// are dummy and this rule equivalent (same produced edge, same preconditions)?
						 contained = true;
						 break;
					 // we also need to consider that the rules may shown the same fact, but with different precondition sets
				 ...
				 for (DependencyRule removeMe : toRemove) rulesProducingEdge.remove(removeMe);
				 if (!contained) rulesProducingEdge.add(dummy);
			 }
		 }
		 return rulesProducingEdge;
	 }
	 
	 public static Set<DependencyRule> getRulesForLoop(SSACFG.BasicBlock loopHead, CGNode node, AbstractDependencyRuleGenerator depRuleGenerator, CallGraph cg) {
		 Util.Unimp("don't call this");
		Set<DependencyRule> loopRules = new TreeSet<DependencyRule>();
		
		Set<DependencyRule> rules = depRuleGenerator.getRulesForNode(node); // add normal loop rules
		for (DependencyRule rule : rules) {
	 ...
	 /**
	  * @param edge - the edge to be checked for produceability in loop
	  * @return true if there are any rules in *any* loop in node that can produce edge; false otherwise
	  */
	 public static boolean isEdgeProduceableByLoop(PointsToEdge edge, CGNode node, HeapGraph hg, AbstractDependencyRuleGenerator depRuleGenerator, CallGraph cg) {
		 ...
		 for (DependencyRule rule : nodeRules) {
			 ...
			 SSACFG.BasicBlock ruleBlk = rule.getBlock();
			 Util.Assert(ruleBlk != null, "no basic block for rule " + rule);
			 if (WALACFGUtil.isInLoopBody(ruleBlk, rule.getNode().getIR())) return true;
		 }
	 	return false;
	 }
	  ...
	  * @param edge - the edge to be checked for produceability in loop
	  * @param loopHead - head of the loop we are looking at
	  * @return true if there are any rules in the loop headed by loopHead that can produce edge; false otherwise
	  */
	 public static boolean isEdgeProduceableByLoop(PointsToEdge edge, SSACFG.BasicBlock loopHead, CGNode node, HeapGraph hg, AbstractDependencyRuleGenerator depRuleGenerator, CallGraph cg) {
		 Util.Unimp("don't call this");
	 ...
	 // wrapper
	 public static List<TreeSet<PointsToEdge>> getNonLoopPreconditionsForEdge(PointsToEdge edge, SSACFG.BasicBlock loopHead, CGNode node, HeapGraph hg, AbstractDependencyRuleGenerator depRuleGenerator, 
				CallGraph cg) {
		 Util.Unimp("don't call this");
		 Util.Debug("getting non-loop preconditions for " + edge);
		 List<TreeSet<PointsToEdge>> pre = getNonLoopPreconditionsForEdge(edge, new TreeSet<PointsToEdge>(), loopHead, node, hg, depRuleGenerator, cg);
	  ...
	  * @param edge - the edge to be checked for produceability in loop
	  * @param loopHead - head of the loop we are looking at
	  * @return the set of preconditions for producing this edge that are *not* contained in the loop
	  */
	 public static List<TreeSet<PointsToEdge>> getNonLoopPreconditionsForEdge(PointsToEdge edge, Set<PointsToEdge> seen, SSACFG.BasicBlock loopHead, CGNode node, HeapGraph hg, AbstractDependencyRuleGenerator depRuleGenerator, 
			 														CallGraph cg) {
		 Util.Unimp("need to account for calls in the loop");
		 Util.Debug("getting non-loop producers for " + edge);
	
		 // TODO: DEBUG
		 if (edge.getSource().isLocalVar()) Util.Assert(edge.getSource().getNode().equals(node), "mismatch: Nodes " + node + " and " + edge.getSource().getNode());
		 ...
		 //Util.Assert(loopProducers.size() < 2, "too many loop producers " + loopProducers.size());
		 
		 for (DependencyRule producer : loopProducers) {
			 List<TreeSet<PointsToEdge>> caseSplits = new LinkedList<TreeSet<PointsToEdge>>();
			 caseSplits.add(new TreeSet<PointsToEdge>()); // start off with no case splits
			 for (PointsToEdge showMe : producer.getToShow()) {
				 if (seen.add(showMe)) { // prevent infinite looping
					 List<TreeSet<PointsToEdge>> choices = getNonLoopPreconditionsForEdge(showMe, seen, loopHead, node, hg, depRuleGenerator, cg);
		 ...
		 for (TreeSet<PointsToEdge> list : preconditionChoices) {
			 if (list.isEmpty()) emptyList = true;
			 Util.Debug("LIST: ");
			 for (PointsToEdge lEdge : list) Util.Debug("edge: " + lEdge);
		 }
		 Util.Debug("done printing lists");
		 if (emptyList) {
			 Util.Debug("found empty precondition set for " + edge);
			 // an empty list means that the edge can be produced entirely in the loop (i.e no non-loop preconditions;
			 // if we can produce an edge with no preconditions, there's no sense in considering ways to produce it which require preconditions
	  ...
	  */
	 /*
	 public static Set<PointsToQuery> getAllLoopQueries(Set<PointsToEdge> initialQuery, SSACFG.BasicBlock loopHead, CGNode node, HeapGraph hg, 
			 											AbstractDependencyRuleGenerator depRuleGenerator, CallGraph cg) {
		 Set<DependencyRule> nodeRules = depRuleGenerator.getRulesForNode(node);
		 HashMap<PointsToEdge,List<DependencyRule>> choices = new HashMap<PointsToEdge, List<DependencyRule>>();
		 for (PointsToEdge edge : initialQuery) {
			 List<DependencyRule> loopProducers = new LinkedList<DependencyRule>();
		 ...
		 if (loopProducers.isEmpty()) return preconditions;
		 // else, 
		 Util.Assert(loopProducers.size() < 2, "too many loop producers " + loopProducers);
		 for (DependencyRule rule : loopProducers) { // for each rule that produces the edge
			 Set<PointsToEdge> toAdd = new TreeSet<PointsToEdge>();
			 for (PointsToEdge showMe : rule.getToShow()) {
				 if (seen.add(showMe)) { // prevent infinite looping
					 preconditions.addAll(getNonLoopPreconditionsForEdge(showMe, seen, loopHead, node, hg, depRuleGenerator, cg));
				 }
			 } //else {
				 //Util.Debug("not in loop " + rule);
				// preconditions.add(edge);
			 //}
		}
	 ...
	 */
	 ...
	 public static PointerVariable makePointerVariableImpl(Object key) {
		 ...
		 if (key instanceof PointerKey) {
			  if (key instanceof LocalPointerKey) {
				  ...
				  //typeId = getIdForType(sfk.getField().getFieldTypeReference().toString());
				  
				  //if (symbolic) return new SymbolicPointerVariable(typeId);
				  //else return new ConcretePointerVariable(key, method, -1, pointerString, typeId);
				  ...
				  //PointerVariable array = Util.makePointerVariable(ack.getInstanceKey());
				  ...
				  //typeId = getIdForType(ack.getInstanceKey().getConcreteType().getReference().toString());
				  //if (symbolic) return new SymbolicPointerVariable(typeId);
				  //else 
			...
			if (key instanceof AllocationSiteInNode) {
				...
				node = as.getNode();
				//pointerString = as.getNode() + "-" + s.getDeclaredType().getName().toString() + "@" + s.getProgramCounter();
				...
				node = nan.getNode();
				//pointerString = nan.getNode() + "-" + nan.getSite().getDeclaredType().getName().toString() + "@" + nan.getSite().getProgramCounter();
				...
				pointerString = site.getMethod() + "-" + site.getSite().getDeclaredType().getName().toString() + "@" + site.getSite().getProgramCounter();
				return new ConcretePointerVariable(site, site.getMethod(), pointerString);//, typeId);
			}
			else if (key instanceof ConcreteTypeKey)
			{
				...
				//if (pointerString.equals("<Primordial,Ljava/lang/String>")) return null;
	 ...
	 public static CGNode getNodeForInstanceKey(InstanceKey key) {
		 if (key instanceof AllocationSiteInNode) return ((AllocationSiteInNode) key).getNode();
		 else if (key instanceof NormalAllocationInNode) return ((NormalAllocationInNode) key).getNode();
		 else if (key instanceof SmushedAllocationSiteInNode) return ((SmushedAllocationSiteInNode) key).getNode();
		 else {
			 Util.Assert(false, "UNIMPLEMENTED INSTANCE KEY! " + key);
			 return null;
		 }
	 ...
	 public static int getSourceLineNumber(IR ir, int instrNum) {
	  ...
	  public static Z3AST makeSymbolicRule(Z3Context ctx, int lhs0, String lhs1,  
			  											  int fieldName0, String fieldName1, 
			  											  String rhs0, String rhs1, 
			  											  int rhsType0, String rhsType1,
			  											  String concreteName,
			  											  String constraintId) {
		  ...
		  // create a propositional variable to represent whether or not variable is bound (starts unbound)
		  ...
		  Z3AST symbTypesEq =  ctx.mkEq(ctx.mkInt(rhsType0, intSort),
  				   						ctx.mkConst(ctx.mkStringSymbol(rhsType1), intSort));
		  ...
		  Z3AST fieldNameEq = ctx.mkEq(ctx.mkInt(fieldName0, intSort),
  				   					   ctx.mkConst(ctx.mkStringSymbol(fieldName1), intSort));
		  Z3AST nonSymbPartEq = ctx.mkEq(ctx.mkInt(lhs0, intSort),
				  						 ctx.mkConst(ctx.mkStringSymbol(lhs1), intSort));
	  ...
	  public static Z3AST makeStrongUpdateRule(Z3Context ctx, int lhs0, String lhs1, int fieldName0, String fieldName1, int rhs0, String rhs1) {
		  Z3Sort intSort = ctx.mkIntSort();
		...
		return
			ctx.mkNot(ctx.mkAnd(ctx.mkEq(ctx.mkInt(lhs0, intSort), 
										 ctx.mkConst(ctx.mkStringSymbol(lhs1), intSort)), 
								ctx.mkEq(ctx.mkInt(fieldName0, intSort),
										 ctx.mkConst(ctx.mkStringSymbol(fieldName1), intSort)),
							    ctx.mkNot(ctx.mkEq(ctx.mkInt(rhs0, intSort),
												   ctx.mkConst(ctx.mkStringSymbol(rhs1), intSort)))));
	  ...
	  /*
	  public static String valueFromPointsToConstraints(String rhs, String fieldName, Set<PointsToEdge> constraints) {
		  for (PointsToEdge edge : constraints) {
			  if (edge.getSource().getName().equals(rhs)) {
				  return edge.getSink().getName();
			  }
		  }
		  return null;
	  }
	  
	  public static boolean constraintsContainLHS(String lhs, Set<PointsToEdge> constraints) {
		  for (PointsToEdge edge : constraints) {
			  if (edge.getSource().getName().equals(lhs)) return true;
		  }
		  return false;
	  }
	  */
	 ...
	 //public static TreeSet<PointsToEdge> deepCopyPointsToEdgeSet(TreeSet<PointsToEdge> edges) {
	 ...
	 public static<K,V> Map<K,V> copyMap(Map<K,V> map) {
	  ...
	  public static <T> TreeSet<T> deepCopyTreeSet(TreeSet<T> set) {
		  ...
		  while (iter.hasNext()) {
			  copy.add(iter.next()); // this is ok since the contents of the set are immutable
		  }
		  return copy;
	  }
	  ...
	  /*
	  private static Z3AST makeNegatedPropositionalVar(String name, Z3Context ctx) {
		  return ctx.mkNot(makePropositionalVar(name, ctx));
	  }
	  */
	  ...
	  public static boolean isBinOpCommutative(BinaryOpInstruction.Operator binOp) {
		  ...
		  switch (binOp) {
			case ADD: commutative = true; break;
			case SUB: break;
			case MUL: commutative = true; break;
			case DIV: break;
			case AND: commutative = true; break;
			case OR: commutative = true; break;
			case XOR: commutative = true; break;
			case REM: break;
		...
		public static BinaryOpInstruction.Operator reverseBinOperator(BinaryOpInstruction.Operator binOp) {
			switch (binOp) {
				case ADD: return BinaryOpInstruction.Operator.SUB;
				case SUB: return BinaryOpInstruction.Operator.ADD;
				case MUL: return BinaryOpInstruction.Operator.DIV;
				case DIV: return BinaryOpInstruction.Operator.MUL;
				case AND: Util.Assert(false, "Not sure how to handle AND!");
				case OR: Util.Assert(false, "Not sure how to handle OR!");
				case XOR: Util.Assert(false, "Not sure how to handle XOR!");
				case REM: Util.Assert(false, "Not sure how to handle REM!");
				default: Util.Assert(false, "Unsupported op!");		
			}
			return binOp;
		}
		...
		public static ConditionalBranchInstruction.Operator flipOperator(ConditionalBranchInstruction.Operator op) {
			switch(op) {
				case LT: return ConditionalBranchInstruction.Operator.GT;
				case LE: return ConditionalBranchInstruction.Operator.GE;
				case GT: return ConditionalBranchInstruction.Operator.LT;
				case GE: return ConditionalBranchInstruction.Operator.LE;
				case EQ: return ConditionalBranchInstruction.Operator.EQ;
				case NE: return ConditionalBranchInstruction.Operator.NE;
				default: Util.Assert(false, "Unsupported op!");			
			}
			return op;
		}
		...
		public static ConditionalBranchInstruction.Operator negateOperator(ConditionalBranchInstruction.Operator op) {
			switch(op) {
				case LT: return ConditionalBranchInstruction.Operator.GE;
				case LE: return ConditionalBranchInstruction.Operator.GT;
				case GT: return ConditionalBranchInstruction.Operator.LE;
				case GE: return ConditionalBranchInstruction.Operator.LT;
				case EQ: return ConditionalBranchInstruction.Operator.NE;
				case NE: return ConditionalBranchInstruction.Operator.EQ;
				default: Util.Assert(false, "Unsupported op!");			
			}
			return op;
		}
		...
		public static PointerVariable makeReturnValuePointer(CGNode callee, HeapModel hm) {
			...
			//return new ConcretePointerVariable(method, makeReturnValueName(method), 0);
		}
		
		/*
		public static String makeReturnValueName(CGNode method) {
			String name = method.toString();
			Util.Assert(!name.contains("synthetic"), "evil synthetic var name " + name);
			return name.toString() + " Retval";
		}
		*/
		...
		public static boolean generateFlowInsensitiveWitness(PointsToEdge edge, Set<PointsToEdge> refuted, AbstractDependencyRuleGenerator depRuleGenerator, 
				   HeapGraph hg, CallGraph cg) {
		...
		public static boolean generateFlowInsensitiveWitness(Set<PointsToEdge> toProduce, Set<PointsToEdge> refuted, AbstractDependencyRuleGenerator depRuleGenerator, 
				   												HeapGraph hg, CallGraph cg) {
			...
			while (!toProduce.isEmpty()) {
				...
				/*
				if (refuted.contains(nextEdge)) {
					Util.Debug("edge " + nextEdge + " already refuted!");
				...
				*/
				...
				for (DependencyRule rule : rules) {
					...
					for (PointsToEdge toShow : rule.getToShow()) {
						...
						if (refuted.contains(toShow)) {
							applicable = false;
							break;
						}
					}
					*/
					if (applicable) applicableRules.add(rule);				
				}
				if (!applicableRules.isEmpty()) {
					...
					for (DependencyRule rule : applicableRules) {
						for (PointsToEdge toShow : rule.getToShow()) {
							if (!produced.contains(toShow)) toProduce.add(toShow);
						}
		...
		public static <T> Set<T> flatten(Collection<Set<T>> sets) {
			...
			for (Set<T> set : sets) flatSet.addAll(set);
			return flatSet;
		}
