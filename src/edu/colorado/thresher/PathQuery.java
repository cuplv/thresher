...
/**
 * Query regarding path feasibility containing formulae acquired from path conditions
 * @author sam
 */
public class PathQuery implements IQuery {
	...
	//final TreeSet<AtomicPathConstraint> constraints; // these need to be ordered for comparison
	final Set<AtomicPathConstraint> constraints; // these need to be ordered for comparison
	
	// set of path constraints that we have had in our constraint set at some point 
	...
	//PathQuery(TreeSet<AtomicPathConstraint> constraints, Set<PointerVariable> pathVars, List<AtomicPathConstraint> witnessList, 
	...
	public PathQuery deepCopy() {
		//return new PathQuery(Util.deepCopyTreeSet(constraints), Util.deepCopySet(pathVars), Util.deepCopyList(witnessList), depRuleGenerator);//, ctx);
		return new PathQuery(Util.deepCopySet(constraints), Util.deepCopySet(pathVars), Util.deepCopyList(witnessList), depRuleGenerator);//, ctx);
	}
	
	@Override
	public boolean containsStaleConstraints(CGNode currentNode) {
		if (Options.DEBUG) Util.Debug("checking for stale constraints before leaving " + currentNode);
		...
		for (PointerVariable var : pathVars) {
			if (var.isLocalVar() && currentNode.equals(var.getNode())) {
				// TODO: hack! for now, we simply drop stale constraints... they can appear in rare cases, such as
				// when a path constraint involves the return value of a call that does not resolve to any call sites.
	...
	public boolean containsLoopProduceableConstraints(SSACFG.BasicBlock loopHead, CGNode currentNode) {
		/*
		Set<DependencyRule> nodeRules = depRuleGenerator.getRulesForNode(currentNode);
	...
	*/
	...
	public boolean visit(SSAArrayLengthInstruction instr, CGNode node) {
		...
		if (pathVars.contains(varName)) {
			SimplePathTerm arrLength = new SimplePathTerm(new ConcretePointerVariable(node, instr.getUse(0), this.depRuleGenerator.getHeapModel()), 
														  SimplePathTerm.LENGTH);
	...
	boolean visitStaticPut(SSAPutInstruction instr, CGNode node, SymbolTable tbl) {
		...
		IField staticField = depRuleGenerator.getCallGraph().getClassHierarchy().resolveField(instr.getDeclaredField());
		if (staticField == null) { // TODO: this shouldn't happen, but it sometimes does. uncomment and try on NPR app punt
			return true;
		}
		PointerVariable staticFieldVar = Util.makePointerVariable(depRuleGenerator.getHeapModel().getPointerKeyForStaticField(staticField));
		if (pathVars.contains(staticFieldVar)) {
			...
			if (tbl.isConstant(use)) {
				if (tbl.isIntegerConstant(use)) { // assigning constant to field
					if (Options.DEBUG) Util.Debug("subbing constant " + tbl.getIntValue(use) + " for " + staticField);
					//substituteExpForVar(new SimplePathTerm(tbl.getIntValue(instr.getUse(1))), staticFieldVar);
	...
	boolean visit(SSAPutInstruction instr, CGNode node, SymbolTable tbl) {
		...
		PointerVariable varName = new ConcretePointerVariable(node, instr.getUse(0), this.depRuleGenerator.getHeapModel()); // non-static field
		if (pathVars.contains(varName)) {
			FieldReference fieldName = instr.getDeclaredField();
			int use = instr.getUse(1);
			if (tbl.isConstant(use)) {
				if (tbl.isIntegerConstant(use)) { // assigning constant to field
					if (Options.DEBUG) Util.Debug("subbing constant " + tbl.getIntValue(use) + " for " + varName + "." + fieldName);
	...
	boolean visit(SSAInvokeInstruction instr, CGNode callee, CGNode caller) {
		if (instr.hasDef()) {
			...
			if (pathVars.contains((returnValue))) {
				//substituteExpForVar(new SimplePathTerm(Util.makeReturnValuePointer(instr.getDeclaredTarget())), returnValue);
				substituteExpForVar(new SimplePathTerm(Util.makeReturnValuePointer(callee, this.depRuleGenerator.getHeapModel())), returnValue);
				return isFeasible();
			}
		}
		return true; // didn't add any constraints, can't be infeasible
		...
		if (resultNum >= 0) { // if the function is a non-void function
			...
			if (pathVars.contains(retvalName)) {
				if (tbl.isConstant(resultNum)) {
					...
					} else if (tbl.isNullConstant(resultNum)) substituteExpForVar(SimplePathTerm.NULL, retvalName);
					else if (tbl.isStringConstant(resultNum)) substituteExpForVar(SimplePathTerm.NON_NULL, retvalName); //Util.Unimp("subbing string consts");
					...
					else Util.Unimp("subbing non-integer constants");
				} else {
	...
	boolean visit(SSANewInstruction instr, CGNode node, SymbolTable tbl) {
		...
		if (pathVars.contains(local)) {
			if (instr.getNewSite().getDeclaredType().isArrayType()) { // special case for arrays
				// may need to update path constraints with the length of this array
				SimplePathTerm arrLength;
				if (tbl.isConstant(instr.getUse(0))) arrLength = new SimplePathTerm(tbl.getIntValue(instr.getUse(0))); 
				else arrLength = new SimplePathTerm(new ConcretePointerVariable(node, instr.getUse(0), this.depRuleGenerator.getHeapModel()));
				substituteExpForFieldRead(arrLength, local, SimplePathTerm.LENGTH);
				
			} else { // not an array
				// all a new() instr can do for us is resolve a constraint of the form "x != null"
	...
	boolean visit(SSABinaryOpInstruction instr, CGNode node, SymbolTable tbl) {
		...
		if (pathVars.contains(varName)) {
			...
			if (!(instr.getOperator() instanceof BinaryOpInstruction.Operator)) {
				if (Options.DEBUG) Util.Debug("dropping constraints due to unsupported op " + instr.getOperator());
				dropConstraintsContaining(varName);
				return true;
			}
			...
			if (op == BinaryOpInstruction.Operator.XOR || op == BinaryOpInstruction.Operator.AND || op == BinaryOpInstruction.Operator.OR) {
				...
				if (Options.DEBUG) Util.Debug("dropping constraints due to unsupported op " + op);
				dropConstraintsContaining(varName);
				return true;
			...
			if (lhsConstant && rhsConstant) { // constants on both sides of operator; evaluate!
				...
				PathTermWithBinOp binExp = new PathTermWithBinOp(tbl.getIntValue(instr.getUse(0)), new ConcretePointerVariable(node, instr.getUse(1), 
																 this.depRuleGenerator.getHeapModel()), op);
				substituteExpForVar(binExp, varName);
			} else if (rhsConstant) { // constant on right of binary operator only
				PathTermWithBinOp binExp = new PathTermWithBinOp(new ConcretePointerVariable(node, instr.getUse(0), this.depRuleGenerator.getHeapModel()), 
																 tbl.getIntValue(instr.getUse(1)), op);
				substituteExpForVar(binExp, varName);
			} else { // no constants
	...
	boolean visit(SSAComparisonInstruction instr, CGNode node, SymbolTable tbl) {
		...
		if (pathVars.contains(varName)) {
			...
			if (tbl.isConstant(use0) && tbl.isConstant(use1)) { // comparison of constants
				Operator op = instr.getOperator();
				if (tbl.isDoubleConstant(use0)) {
					switch (op) {
						case CMP: return tbl.getDoubleValue(use0) == tbl.getDoubleValue(use1);
						case CMPL: return tbl.getDoubleValue(use0) < tbl.getDoubleValue(use1);
						case CMPG: return tbl.getDoubleValue(use0) > tbl.getDoubleValue(use1);
						default: Util.Assert(false, "UNSUPPORTED OPERATOR " + op + " IN COMPARISON");
					}
				} else if (tbl.isFloatConstant(use0)) {
					switch (op) {
						case CMP: return tbl.getFloatValue(use0) == tbl.getFloatValue(use1);
						case CMPL: return tbl.getFloatValue(use0) < tbl.getFloatValue(use1);
						case CMPG: return tbl.getFloatValue(use0) > tbl.getFloatValue(use1);
						default: Util.Assert(false, "UNSUPPORTED OPERATOR " + op + " IN COMPARISON");
					}
			  } else if (tbl.isLongConstant(use0)) {
				  switch (op) {
					case CMP: return tbl.getLongValue(use0) == tbl.getLongValue(use1);
					case CMPL: return tbl.getLongValue(use0) < tbl.getLongValue(use1);
					case CMPG: return tbl.getLongValue(use0) > tbl.getLongValue(use1);
					default: Util.Assert(false, "UNSUPPORTED OPERATOR " + op + " IN COMPARISON");
				  }
			  } else Util.Unimp("unexpected const type in comparison " + instr);
			} else { // not a constant 
				// TODO: we don't currently support this case... can only store integer constants in path constraints.
	...
	void dropConstraintsContaining(PointerVariable varName) {
		...
		for (AtomicPathConstraint constraint : this.constraints) {
			if (constraint.getVars().contains(varName)) toRemove.add(constraint);
		}
		for (AtomicPathConstraint constraint : toRemove) {
			if (Options.DEBUG) Util.Debug("dropping constraint " + constraint);
	...
	boolean visit(SSAArrayLoadInstruction instr, CGNode node, SymbolTable tbl) {
		 PointerVariable varName = new ConcretePointerVariable(node, instr.getDef(), this.depRuleGenerator.getHeapModel());
		 if (pathVars.contains(varName)) {
			 // can model array write as (array name).array.i, where i is the index of interest
			 // TODO: add constraints for array loads
			 //Util.Unimp("path queries with arrays. this arrayLoad insruction " + instr + " might matter for " + this);
			 if (Options.DEBUG) Util.Debug("we don't handle path queries with arrays precisely; dropping constraints. this arrayLoad insruction " 
	...
	boolean visit(SSAArrayStoreInstruction instr, CGNode node, SymbolTable tbl) {
		 ...
		 if (pathVars.contains(stored)) {
			 ...
			 //Util.Unimp("path queries with arrays. this arrayStore instruction " + instr + " might matter for " + this);
			 if (Options.DEBUG) Util.Debug("we don't handle path queries with arrays precisely; dropping constraints. this arrayStore insruction " 
					 	+ instr + " might matter for " + this);
	...
	public boolean visit(SSAInstanceofInstruction instr, CGNode node) {
		PointerVariable lhsVar = new ConcretePointerVariable(node, instr.getDef(), this.depRuleGenerator.getHeapModel());
		if (pathVars.contains(lhsVar)) {
			// TODO: we don't have enough type information to check this constraint right now, so just drop it
	...
	// TODO: track catches so that thrown exceptions aren't necessarily an instant refutation
	public boolean visit(SSAThrowInstruction instr, CGNode node) {
		if (Options.DEBUG) Util.Debug("refuted by thrown exception!");
		this.feasible = false;
		return false;
	}
	...
	void substituteExpForVar(PathTerm toSub, PointerVariable subFor) {
		if (Options.DEBUG) Util.Debug("subsExpForVar subbing " + toSub + " for " + subFor);
		...
		for (AtomicPathConstraint constraint : constraints) {
			...
			if (toSub instanceof SimplePathTerm && !toSub.isIntegerConstant()) newConstraint = constraint.heapSubstitute((SimplePathTerm) toSub, subForTerm);
			else newConstraint = constraint.substitute(toSub, subForTerm);
			if (newConstraint.substituted()) {
				...
				if (newConstraint != AtomicPathConstraint.TRUE) toAdd.add(newConstraint);
		...
		for (AtomicPathConstraint constraint : toAdd) addConstraint(constraint);
	}
	...
	boolean addConstraint(AtomicPathConstraint constraint) {	
		...
		if (constraints.add(constraint)) {
			/*
			// DEBUG
			Iterator<AtomicPathConstraint> iter = constraints.descendingIterator();
			AtomicPathConstraint last = null;
			while (iter.hasNext()) {
				AtomicPathConstraint cur = iter.next();
				if (last != null) Util.Assert(last.compareTo(cur) >= 0, "compare invariant broken for " + Util.printCollection(this.constraints) + "\n" + last + " < " + cur);
				last = cur;
			}
			// END DEBUG
			*/
	...
	boolean removeConstraint(AtomicPathConstraint constraint) {
		if (constraints.remove(constraint)) {
			...
			Util.Unimp("couldn't remove " + constraint + " from " + Util.printCollection(this.constraints) + " contains? " + constraints.contains(constraint));
		}
		return false;
	}
	...
	public List<IQuery> returnFromCall(SSAInvokeInstruction instr, CGNode callee, IPathInfo currentPath) {
		...
		if (WALACFGUtil.isConstructor(instr)) { // if this is a constructor
			...
			// returning from constructors is a special case because we have to initialize all untouched fields to their default values
			PointerVariable thisVar = new ConcretePointerVariable(callee, 1, this.depRuleGenerator.getHeapModel()); // the "this" var is always v1
			...
			for (AtomicPathConstraint constraint : constraints) {
				if (constraint.getLhs() instanceof SimplePathTerm) {
					...
					// we model 0 == null == false all as the integer 0. each is the default value for its respective type
		...
		if (!substituteActualsForFormals(instr, currentPath.getCurrentNode(), callee,
	...
	public boolean addConstraintFromBranchPoint(IBranchPoint point, boolean trueBranchFeasible) {
		...
		if (instruction.isIntegerComparison() && tbl.isConstant(instruction.getUse(0)) && tbl.isConstant(instruction.getUse(1))) {
			// yes, so we can determine immediately whether this branch can be taken (no need to add path constraints). 
			return evaluateGuard(instruction, tbl, !trueBranchFeasible); // we should negate the path constraint if the true branch is infeasible
			...
			return true; // else, constraint already in set; no need to check feasibility
		} 
	}
	
	/**
	 * @param instruction - instruction containing comparison
...
//		Util.Assert(instruction.isIntegerComparison(), "FOUND NON INTEGER COMPARISON");
		...
		if (instruction.isIntegerComparison() || (tbl.isBooleanOrZeroOneConstant(use0) && tbl.isBooleanOrZeroOneConstant(use1))) {
			...
			switch (op) {
			  	case LE: result = lhsValue <= rhsValue; break;
			  	case LT: result = lhsValue < rhsValue; break;
			  	case EQ: result = lhsValue == rhsValue; break;
			  	case NE: result = lhsValue != rhsValue; break;
			  	case GE: result = lhsValue >= rhsValue; break;
			  	case GT: result = lhsValue > rhsValue; break; 
			  	default: Util.Assert(false, "UNSUPPORTED OPERATOR " + op + " IN GUARD");
			}
		} else if (tbl.isNullConstant(use0) && tbl.isNullConstant(use1)) {
			switch (op) {
			  	case EQ: result = true; break;
			  	case NE: result = false; break;
			  	default: Util.Assert(false, "UNSUPPORTED OPERATOR " + op + " for null comparison IN GUARD");
			}
			...
			switch (op) {
			  	case EQ: result = lhsValue.equals(rhsValue); break;
			  	case NE: result = !lhsValue.equals(rhsValue); break;
			  	default: Util.Assert(false, "UNSUPPORTED OPERATOR " + op + " IN GUARD");
			}
		} else {
	...
	/**
	 * @param negate - should we flip the operator in the instruction?
	 * @return path constraint extracted from instruction
	 */
	AtomicPathConstraint getPathConstraintFromGuard(SSAConditionalBranchInstruction instruction, SymbolTable tbl, CGNode node, boolean negate) {
		...
		if (negate) op = Util.negateOperator((ConditionalBranchInstruction.Operator) instruction.getOperator());
		else op = (ConditionalBranchInstruction.Operator) instruction.getOperator();
		...
		} else if (tbl.isConstant(use0) || tbl.isConstant(use1)){ // one or both vars are some kind of non-int constant
	...
	/**
	 * substitute actuals for formals in our constraint set (i.e., when returning from call)
	 * @param callerSymbolTable - symbol table for the caller
	 */
	boolean substituteActualsForFormals(SSAInvokeInstruction instr, CGNode callerMethod, CGNode calleeMethod, 
		...
		for (int i = 0; i < instr.getNumberOfParameters(); i++) {
			...
			if (callerSymbolTable.isIntegerConstant(use)) actual = new SimplePathTerm(callerSymbolTable.getIntValue(use));
			else if (callerSymbolTable.isNullConstant(use)) actual = SimplePathTerm.NULL;
			else if (callerSymbolTable.isStringConstant(use)) actual = SimplePathTerm.NON_NULL; // the only modeling we do of strings is that they are non-null
			...
			else actual = new SimplePathTerm(new ConcretePointerVariable(callerMethod, instr.getUse(i), this.depRuleGenerator.getHeapModel())); 
			if (Options.DEBUG) Util.Debug("subbing " + actual + " for " + formal);
	...
	boolean substituteFormalsForActuals(SSAInvokeInstruction instr, CGNode callerMethod, CGNode calleeMethod) {
		...
		for (int i = 0; i < instr.getNumberOfParameters(); i++) {
			...
			if (this.pathVars.contains(actual)) {
				Util.Debug("path vars have actual");
				for (AtomicPathConstraint constraint : this.constraints) { // check if this formal is our path constraints
					// if it is, it's important to use this copy of the var, since it may have an associated heap location that we need to remember
					if (constraint.getLhs().getVars().contains(actual)) {
						Set<PointerVariable> locs = constraint.getLhs().getHeapLocs();
						if (locs.isEmpty()) break;
						// if this fires, it just means we have to drill deeper and find the exact SimplePathTerm corresponding to heap loc
						Util.Assert(locs.size() < 2, "not sure what to do with multiple locs here");
						PointerVariable loc = locs.iterator().next();
						Util.Debug("replacing " + actual + " with " + formal + " and associated " + loc);
						formalTerm = new SimplePathTerm(formal, loc);
						break;
					} else if (constraint.getRhs().getVars().contains(actual)) {
						
						Set<PointerVariable> locs = constraint.getRhs().getHeapLocs();
						if (locs.isEmpty()) break;
						Util.Assert(locs.size() < 2, "not sure what to do with multiple locs here");
						PointerVariable loc = locs.iterator().next();
						Util.Debug("replacing " + actual + " with " + formal + " and associated " + loc);
	...
	public List<IQuery> visitPhi(SSAPhiInstruction instr, int phiIndex, IPathInfo currentPath) {
		...
		PointerVariable lhsVar = new ConcretePointerVariable(currentMethod, instr.getDef(), this.depRuleGenerator.getHeapModel()); // the x in x = phi(y,z)
		if (pathVars.contains(lhsVar)) {
			...
			SimplePathTerm toSub = null;
			if (tbl.isIntegerConstant(use)) toSub = new SimplePathTerm(tbl.getIntValue(use));
			else if (tbl.isNullConstant(use)) toSub = SimplePathTerm.NULL;
			else if (tbl.isStringConstant(use)) toSub = SimplePathTerm.NON_NULL; // the only modeling we do of strings is that they are non-nul
			else if (tbl.isConstant(use)) Util.Unimp("other kinds of constants"); // TODO: support other constants
			// one of the y_i's in x = phi(y_1,y_2,...)
			else toSub = new SimplePathTerm(new ConcretePointerVariable(currentMethod, use, this.depRuleGenerator.getHeapModel())); 
			substituteExpForVar(toSub, lhsVar); // sub the LHS of the phi for the appropriate term on the right
			if (!isFeasible()) {
				this.feasible = false;
				return IQuery.INFEASIBLE;
			}
	...
	@Override
	public List<IQuery> visit(SSAInstruction instr, IPathInfo currentPath, Set<PointsToEdge> refuted) { return visit(instr, currentPath); }
	...
	public List<IQuery> visit(SSAInstruction instr, IPathInfo currentPath) {
		...
		if (instr instanceof SSAArrayStoreInstruction) result = visit((SSAArrayStoreInstruction) instr, node, tbl);
		//else if (instr instanceof SSAAbstractThrowInstruction) return query.visit((SSAAbstractThrowInstruction) instr);
		else if (instr instanceof SSAUnaryOpInstruction) result = visit((SSAUnaryOpInstruction) instr, node);
		//else if (instr instanceof SSAAddressOfInstruction) return query.visit((SSAAddressOfInstruction) instr);
		else if (instr instanceof SSAArrayLengthInstruction) result = visit((SSAArrayLengthInstruction) instr, node);
		else if (instr instanceof SSAArrayLoadInstruction) result = visit((SSAArrayLoadInstruction) instr, node, tbl);
		else if (instr instanceof SSABinaryOpInstruction) result = visit((SSABinaryOpInstruction) instr, node, tbl);
		else if (instr instanceof SSACheckCastInstruction) result = visit((SSACheckCastInstruction) instr, node);
		
 		//else if (instr instanceof SSAConditionalBranchInstruction) return query.visit((SSAConditionalBranchInstruction) instr);
		else if (instr instanceof SSAGetInstruction) result = visit((SSAGetInstruction) instr, node);
 		else if (instr instanceof SSAPutInstruction) result = visit((SSAPutInstruction) instr, node, tbl);
 		else if (instr instanceof SSAGotoInstruction) return IQuery.FEASIBLE; // goto's are a nop for us
 		else if (instr instanceof SSASwitchInstruction) return IQuery.FEASIBLE; // switch is a nop
 		else if (instr instanceof SSALoadMetadataInstruction) return IQuery.FEASIBLE; // this is a nop, I think
 		else if (instr instanceof SSAConversionInstruction) return IQuery.FEASIBLE; // we don't reason about this; it's a nop
 		else if (instr instanceof SSAInstanceofInstruction) result = visit((SSAInstanceofInstruction) instr, node);
 		else if (instr instanceof SSAComparisonInstruction) result = visit((SSAComparisonInstruction) instr, node, tbl);
 		else if (instr instanceof SSAMonitorInstruction) return IQuery.FEASIBLE; // have no idea what this is. nop!
 		else if (instr instanceof SSAGetCaughtExceptionInstruction) return IQuery.FEASIBLE; // TODO: handle this properly 
 		else if (instr instanceof SSAThrowInstruction) result = visit((SSAThrowInstruction) instr, node);
 		...
 		else if (instr instanceof SSANewInstruction) result = visit((SSANewInstruction) instr, node, tbl);
		else if (instr instanceof SSAReturnInstruction) result = visit((SSAReturnInstruction) instr, node, tbl); 
		//else if (instr instanceof SSAStoreIndirectInstruction) return query.visit((SSAStoreIndirectInstruction) instr);
		...
		AtomicPathConstraint last = null;
		while (iter.hasNext()) {
			AtomicPathConstraint next = iter.next();
			if (last != null) {
				Util.Assert(last.compareTo(next) > 0, "constraint " + last + " not greater than " + next);
			}
			last = next;
		}
		*/
	...
	public boolean symbContains(PathQuery other) {
		...
		// prevent z3 from giving a satisfying assignment that makes left side of implication false
		...
		//return tmp.check();
		/*
		// TODO: rather than doing this check, should just use Z3 to check implication here
		if (other.constraints.isEmpty()) return true;
		if (this.constraints.isEmpty()) return false;
		...
		for (AtomicPathConstraint constraint1 : other.constraints) { // for each constraint in other
			boolean contained = false;
			for (AtomicPathConstraint constraint2 : this.constraints) { // for each constraint in this
	...
	public void removeAllLocalConstraints() {
		...
		for (AtomicPathConstraint constraint : constraints) {
			for (PointerVariable var : constraint.getVars()) {
				if (var.isLocalVar()) toRemove.add(constraint);
				break;
			}
		}
		for (AtomicPathConstraint constraint : toRemove) {
	...
	public void intializeStaticFieldsToDefaultValues() {
		...
		for (PointerVariable var : pathVars) {
			if (var.getInstanceKey() instanceof StaticFieldKey) toSub.add(var);
		}
	...
	/**
	 * return the set of methods that can potentially produce / affect some part of this query
	 */
	@Override
	...
	public List<AtomicPathConstraint> getUnsatCore() {
		...
		// return core if we've already constructed it via evaluating a constraint to false
		if (unsatCore != null) return unsatCore; 
		...
		//this.ctx.checkAssumptions(this.currentPathAssumption, new Z3Model(), core);
		return null;
	}
	...
	public boolean containsConstraint(Constraint constraint) {
		if (!(constraint instanceof AtomicPathConstraint)) return false;
		/*
		int id = ((AtomicPathConstraint) constraint).getId();
		// TODO: this can be made more efficient...
		for (AtomicPathConstraint con : this.constraints) {
			if (con.getId() == id) return true;
		}
		return false;
		*/
	...
	public boolean contains(IQuery other) {
		if (!(other instanceof PathQuery)) return false;
		PathQuery otherQuery = (PathQuery) other;
	...
	public int compareTo(Object other) {
		if (!(other instanceof PathQuery)) Util.Unimp("comparing to non- points-to query");
		PathQuery otherQuery = (PathQuery) other;
		TreeSet<AtomicPathConstraint> otherConstraints = otherQuery.constraints;
		...
		// sizes are the same; do constraint-by-constraint comparison
		final Iterator<AtomicPathConstraint> ptIter = constraints.descendingIterator();
		final Iterator<AtomicPathConstraint> otherPtIter = otherConstraints.descendingIterator();
		while (ptIter.hasNext() && otherPtIter.hasNext()) {
			final AtomicPathConstraint edge0 = ptIter.next();
			final AtomicPathConstraint edge1 = otherPtIter.next();
			final int comparison = edge0.compareTo(edge1);
			if (comparison != 0) return comparison;
		}
		return 0; // all constraints compared equal
	}
	*/
