...
public class Options {
	...
	public static boolean CHECK_ASSERTS = true; // crashes when assertion checks fail if true

	@boolOpt( description="log extra info (very little printing. this should probably go away)", _default=true )
	public static boolean LOG = true; // log extra info (very little printing. this should probably go away)

	@boolOpt( description=" print important output stats (probably shouldn't turn off)", _default=true )
	public static boolean PRINT = true; // print important output stats (probably shouldn't turn off

	@boolOpt( description="give up after assertion failure / crash", _default=true )
	public static boolean EXIT_ON_FAIL = true; // give up after assertion failure / crash if true
	...
	public static final boolean DUMP_WITNESSED_ERR_PATHS = true; // prints end-to-end list of witnessed errors in heap
	
	// if true, generate dependency rules for all statements in a method when we generate rules for one statement in that method (cache all rules)
	// otherwise, generate dependency rules for each statement as we encounter it (though for checking relevance of callees,
	// we still do whole-method rule generation)
	//@boolOpt( description="generate dependency rules for all statements in a method when we generate rules for one statement in that method", _default=false )
	public static final boolean GEN_DEPENDENCY_RULES_EAGERLY = false; // you want this off to be fast
	...
	// not currently enabled
	//@boolOpt (description="do summaries do smart subsumption check (true), or dumb equality check (false)?", _default=true ) 
	public static final boolean SUBSUMPTION_CHECK_AT_SUMMARIES = true; // you want this on to be fast

	// not currently enabled
	//@boolOpt (description="infer invariants for relevant loops? (alternative: drop all constraints produced in loop)", _default=true)
	public static final boolean LOOP_INVARIANT_INFERENCE = true; // you want this on to be precise	
	...
	// if the paths to explore stack ever grows larger than this, we report a timeout 
	...
	//private static final boolean GEN_FLOW_INSENSITIVE_WITNESS = false; // insist on generating flow-insensitive witness before doing symbolic execution (TODO: doesn't work)
	
	// return path to dir to perform analyis on or _REGRESSION if regressions
	public static String parseArgs(String[] args) {
		if (args.length == 0) dumpHelpInfo();
		int index = 0;
		while (index < args.length) {
			...
			if (arg == "") { index++; continue; }
				...
				arg = arg.substring(1);
				// special cases
				if (arg.equals("help")) dumpHelpInfo();
		...
		for (Field field : Options.class.getDeclaredFields()) {
			...
			if (annots.length > 0) {
				if (annots[0] instanceof stringOpt) {
					...
					if (opt._default() != null) optionStr += "\tdefault: " + opt._default();
					optionStr += "\n";
