...
public class PiecewiseSymbolicExecutor extends PruningSymbolicExecutor {
  ...
  public boolean handlePiecwiseExecutionMethodBased(final IPathInfo path) {
    ...
    for (CGNode producer : producers) {
      ...
      /*
       * // unsound because we may have started execution in a part of startNode
       * that precedes the call to producer // if the producer is called
       * exclusively by start node if (onlyCalledBy(producer, startNode)) {
      ...
      if (producer.getMethod().isClinit()) { // producer is class initializer;
      ...
      if (feasiblePathExists(producer, startNode)) {
        ...
        for (Iterator<CGNode> preds = callGraph.getPredNodes(producer); preds.hasNext();) {
          ...
          newPath.addContextualConstraints(producer); // needed to prevent false
          ...
          if (executeToProcedureBoundary(newPath, producer)) {
            ...
            Util.Debug("refuted! " + producer + " from " + startNode + "at depth " + depth + " trying next producer"); // TODO:
                                                                                                                       // disruptor
                                                                                                                       // check!
        ...
        Util.Debug("no feasible path between " + producer + " and " + startNode); // else,
                                                                                  // no
                                                                                  // path
                                                                                  // between
                                                                                  // producer
  ...
  /*
   * public boolean handlePiecewiseExecution(final IPathInfo path) { final
   * CGNode startNode = path.getCurrentNode(); Util.Debug("start node is " +
   * startNode); final IPathInfo copy = path.deepCopy();
   * copy.removeAllLocalConstraintsFromQuery(); // eliminate all non-heap
   * constraints (should only be constraints on function params)
   * Util.Debug("after removing all local constraints: " + copy); if
   * (copy.foundWitness()) { // check to make sure we didn't remove everything
   * Util.Debug("all non-local constraints produced; returning witness");
   * path.declareFakeWitness(); return true; } // get potential producers for
   * constraints Map<Constraint,Set<CGNode>> queryModMap =
   * copy.getModifiersForQuery();
   * 
   * IPathInfo classInitPath = copy.deepCopy(); // handle class initializer case
   * first
   * classInitPath.setCurrentNode(WALACFGUtil.getFakeWorldClinitNode(this.callGraph
   * )); Util.Debug("about to try fakeWorldClinit"); boolean result =
   * this.handleFakeWorldClinit(classInitPath); if (result) return true; //
   * found witness in fakeWorldClinit // else, refuted; try other producers
   * 
   * // this is an optimization to find refutations quickly, but in the case
   * that it fails, we should (maybe) fall back on the regular symbolic executor
   * for (Constraint constraint : queryModMap.keySet()) { // for each
   * (constraint, modifying methods pair)
   * Util.Debug("starting piecewise execution to produce/refute " + constraint +
   * " from " + startNode); boolean feasibleOutcome = false; Set<CGNode>
   * producers = queryModMap.get(constraint); Util.Assert(!producers.isEmpty(),
   * "found no producers for " + constraint); for (CGNode producer : producers)
   * { // for each potential producer for the constraint
   * Util.Debug("trying node " + producer); IPathInfo newPath = copy.deepCopy();
   * // do callgraph feasability checks
   * 
   * // TODO: this is unsound -- figure out what we should really do here. //
   * (1) is the producer the same as the current node or called by the current
   * node? if (startNode.equals(producer)) {// || callGraphPathExists(startNode,
   * producer, callGraph)) { // yes; don't want to consider the effect of
   * calling this twice
   * Util.Debug("start node same as producer or calls producer, continuing");
   * continue; }
   * 
   * // is the start node reachable from the producer? if
   * (superGraphPathExists(producer, startNode)) { Util.Debug(
   * "producer and startNode share common caller; can enter producer as callee"
   * ); Iterator<CGNode> preds = callGraph.getPredNodes(producer); // is
   * producer fakeWorldClinit or some other top-level node?
   * Util.Assert(preds.hasNext(), "no preds for " + producer);
   * 
   * CGNode producerCaller = preds.next(); Util.Assert(!preds.hasNext(),
   * "producer can be called from more than one place!"); // common ancestors
   * non-empty; nodes share common caller. can enter from exit block of producer
   * SSAInvokeInstruction callInstr =
   * WALACFGUtil.getCallInstructionFor(producer, producerCaller, callGraph);
   * newPath.addContextualConstraints(producer); // needed to prevent false
   * refutations based on stale constraints
   * newPath.setCurrentNode(producerCaller);
   * newPath.enterCallFromJump(callInstr, callGraph, producer); if
   * (executeToProcedureBoundary(newPath, producer, constraint)) { if
   * (newPath.foundWitness()) { path.declareFakeWitness(); return true; }
   * feasibleOutcome = true; break; } else { // refutation! need to do disruptor
   * check + try next Util.Debug("refuted! " + producer + " from " + startNode +
   * " trying next producer"); // TODO: disruptor check! // allow to fall
   * through to next continue; //Util.Unimp("disruptor check"); // can get
   * refutation constraint as follows: // startConstraints \ (currentConstraints
   * U currentProduced) } } // else, no path between producer and startNode in
   * the callgraph } // end for each potential producer
   * 
   * if (!feasibleOutcome) { // no feasible outcomes for this constraint
   * Util.Debug("no feasible outcomes for " + constraint + "!");
   * Util.Assert(this.pathsToExplore.peek().isDummy(), "found non-dummy path! "
   * + this.pathsToExplore.peek());
   * Util.Assert(this.branchPointStack.peek().isDummy(),
   * "found non-dummy branch point! " + this.branchPointStack.peek()); return
   * false; } Util.Debug(constraint + " can be produced without refutation.");
   * // clear the pathsToExplore and branchPointStack back to the dummy while
   * (!this.pathsToExplore.peek().isDummy()) this.pathsToExplore.remove(); while
   * (!this.branchPointStack.peek().isDummy()) this.branchPointStack.remove(); }
   * // end for each (constraint, modifying method) pair return true; // no
   * attempts produced definite refutation }
   */
  ...
  public boolean handleInterproceduralExecution(IPathInfo path) {
    ...
    if (isAbstractionBoundary(startMethod)) { // is this method at an
                                              // abstraction boundary?
      Util.Debug("at abstraction boundary. going to do piecewise execution");
      addPathAndBranchPlaceholders();
      boolean result = handlePiecwiseExecutionMethodBased(path);// handlePiecewiseExecution(path);
                                                                // // do
                                                                // piecewise
                                                                // exec
    ...
    while (true) { // until we're done exploring this call
      ...
      if (this.executeBackwardsPathIntraprocedural(path)) {
        if (path.foundWitness()) {
          ...
          inputPath.declareFakeWitness(); // make sure caller knows we found a
                                          // witness
          return true; // found witness
        } // else, procedure boundary has been reached

        if (!path.isCallStackEmpty()) { // returned from call, but not back to
        ...
        if (proc.getMethod().isInit()) { // special case for constructors; need
                                         // to fake returning from call
          ...
          Iterator<CallSiteReference> iter = caller.iterateCallSites(); // pick
                                                                        // any
                                                                        // call
                                                                        // site
          ...
          SSAInvokeInstruction call = (SSAInvokeInstruction) instrs[caller.getIR().getCallInstructionIndices(site).intIterator()
              .next()]; // pick any call
          ...
          // else, try executing the class initializer to see if that produces a
          // refutation
        ...
        if (toProduce != null) {
          if (path.containsConstraint(toProduce)) { // constraint was not
                                                    // produced if path still
                                                    // contains it
        ...
        if (isAbstractionBoundary(path.getCurrentNode().getMethod())) {
          ...
          // if (isPathInSummary(path)) return false; // summary makes path
          // redundant
          // if (count > 5) {
          ...
          boolean result;
          if (toProduce != null)
            result = handlePiecwiseExecutionMethodBased(path);
          else
            result = handlePiecwiseExecutionMethodBased(path);
          if (result) {
        ...
        for (IPathInfo caseSplit : caseSplits) {
          if (executeToProcedureBoundary(caseSplit, caseSplit.getCurrentNode(), toProduce, false))
            return true;
        }
        return false;
      } else { // path refuted
  ...
  private IPathInfo getNextPath() {
    ...
    } else if (!branchPointStack.peek().isDummy()) { // any branch points left
                                                     // to explore?
    ...
    // Util.Assert(path != null || (pathsToExplore.peek().isDummy() &&
    // branchPointStack.peek().isDummy()), "neglected to pop non-dummy!");
    return path;
  }
  ...
  private void handleCallCase(CGNode startNode) {
    ...
    while (callers.hasNext()) {
      ...
      List<ISSABasicBlock> callBlks = new LinkedList<ISSABasicBlock>(); // the
                                                                        // blocks
                                                                        // that
                                                                        // call
                                                                        // our
     ...
     * interCFG.getCFG(this.callGraph.getFakeRootNode()); SSAInstruction[]
     * instrs = cfg.getInstructions(); Util.Debug(instrs.length + " instrs.");
     * for (int i = 0; i < instrs.length; i++) { if (instrs[i] instanceof
     * SSAInvokeInstruction) { SSAInvokeInstruction invoke =
     * (SSAInvokeInstruction) instrs[i]; Util.Debug("INVOKE " + invoke); if
     * (invoke.getDeclaredTarget().equals(startNode.getMethod().getReference()))
     * { Util.Assert(false, "found it!"); } } }
     */
    // MyBFSPathFinder<CGNode> finder = new MyBFSPathFinder<CGNode>(cfg,
    // startNode, startNode);
  }
