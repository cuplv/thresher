...
public class ConcretePointerVariable implements PointerVariable { //implements Comparable {
	...
	//public final static ConcretePointerVariable NON_NULL = new ConcretePointerVariable(-2);
	...
	// special tag corresponding to instance number of constraint. introduced only when needed
	...
	//public ConcretePointerVariable(CGNode node, int useNum, AbstractDependencyRuleGenerator gen, int nodeNum) {
	public ConcretePointerVariable(CGNode node, int useNum, HeapModel hm) {
		...
		//if (useNum != -1) Util.Assert(name.indexOf('@') == -1, "weird var name " + name + " " + useNum);
		//this.instanceKey = gen.getHeapModel().getPointerKeyForLocal(node, useNum);
		this.instanceKey = hm.getPointerKeyForLocal(node, useNum);
		
		//this.id = gen.getHeapGraph().getNumber(instanceKey);
		
		//Util.Assert(instanceKey != null, "couldn't find pointerkey for " + node + " -v " + useNum);
		this.id = instanceKey.toString().hashCode();//Util.getIdForVar(name);
		//Util.Assert(((LocalPointerKey) instanceKey).getNode().equals(node), "nodes don't match!");
	...
	public PointerVariable deepCopy() {
		// all the fields are immutable or primitive, so there's no need to make a copy
		return this;
	}
	/*
	...
	*/
	...
	@Override
	public Set<InstanceKey> getPossibleValues() { return null; }
	
	public String toString() {
		// note than changing between forms doesn't affect the correctness of the program; only the readability of the error reports!
		// human-readable form; may make vars from different methods appear to be the same var
	...
	public String toHumanReadableString() {
		if (instanceKey instanceof LocalPointerKey) {
			...
			return lpk.getNode().getMethod().getName().toString() + "-v" + lpk.getValueNumber();// + " R: " + lpk.getNode().getContext().get(ContextKey.RECEIVER);
		} else if (instanceKey instanceof AllocationSiteInNode) {
			AllocationSiteInNode alloc = (AllocationSiteInNode) instanceKey;
			//return node.getMethod().getName() + "-" + alloc.getSite().toString() + " R: " + alloc.getNode().getContext().get(ContextKey.RECEIVER);
			return node.getMethod().getName() + "-" + alloc.getSite().toString();// + " R: " + alloc.getNode().getContext().get(ContextKey.RECEIVER);
	...
	public boolean symbContains(PointerVariable other) {
		if (other instanceof SymbolicPointerVariable) {
			SymbolicPointerVariable symb = (SymbolicPointerVariable) other;
			return symb.getPossibleValues().size() == 1 && 
				   symb.getPossibleValues().contains(this.instanceKey);
		} else if (other instanceof ConcretePointerVariable) {
			return this.equals(other);
		}
		return false;
	...
	public int compareTo(Object other) {
		if (other instanceof SymbolicPointerVariable) return 1;
		else if (other instanceof ConcretePointerVariable) {
	...
	public boolean equals(Object other) {
		if (other instanceof ConcretePointerVariable) {
			...
			/*
			//			return this.id == p.id;
			if (instanceKey != null && p.instanceKey != null) {
				//return instanceKey.toString().equals(p.instanceKey.toString());
				return this.id == p.id;
			} else if (this.method != null && p.method != null) {
				Util.Assert(false, "bad pointer vars " + other + " " + this);
				return this.method.toString().equals(p.method.toString());
				//return this.method.hashCode() == p.method.hashCode();
			}
	...
	public IMethod getMethod() { return method; }
	...
	public boolean isHeapVar() { 
		return instanceKey != null && (instanceKey instanceof AllocationSite || instanceKey instanceof AllocationSiteInNode ||
										instanceKey instanceof SmushedAllocationSiteInNode || instanceKey instanceof ArrayContentsKey
										|| instanceKey instanceof StaticFieldKey);
	}
	
	@Override
	public int hashCode() {
		//String hashString = this.method.toString() + " " + this.name + " " + this.instanceNum;
		//return hashString.hashCode();
		//return name.hashCode();
		if (instanceKey != null) return instanceKey.hashCode(); //return instanceKey.toString().hashCode();
		return name.hashCode();
	}
	...
	*/
	...
	public String parseToHumanReadable(String str) {
		...
		if (str == null || str.contains("symb") || !str.contains("-") || str.equals("-1")) return str; 
